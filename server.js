...
const db_2 = require("./src/server/db");
const form_data_1 = __importDefault(require("form-data"));
const nodemailer_1 = __importDefault(require("nodemailer"));
const crypto_1 = __importDefault(require("crypto"));
const sharp_1 = __importDefault(require("sharp"));
const cleanup_expired_images_1 = require("./src/server/cleanup-expired-images");
...
setInterval(() => {
    ...
    for (const [key, val] of proxyImageCache) {
        if (now - val.timestamp > PROXY_CACHE_TTL)
...
// 模型名称规范化（用于前端展示）
function normalizeModelForDisplay(rawModel) {
    if (!rawModel)
        return 'nano';
    // 移除可能已有的自动删除后缀
    const clean = rawModel.replace(/ · \d+天后自动删除| · 即将删除/g, '').trim();
    if (clean === 'gemini-3.1-flash-image-preview' || clean === 'nanobann2' || clean === 'nanogen')
        return 'nano';
    return clean || 'nano';
}
// 计算图片剩余天数（生成后 3 天自动删除）
function getAutoDeleteInfo(createdAt) {
    if (!createdAt)
        return ' · 3天后自动删除';
...
// 获取价格配置（带缓存）
async function getPricing(key) {
    const now = Date.now();
    if (now - pricingCacheTime > PRICING_CACHE_TTL || !pricingCache.has(key)) {
        try {
            const [rows] = await db_2.pool.execute('SELECT price, enabled FROM pricing_config WHERE `key` = ?', [key]);
...
async function getUserEffectivePricing(userId, pricingKey) {
    const defaultPrice = await getPricing(pricingKey);
    try {
        const [users] = await db_2.pool.execute('SELECT invited_by FROM users WHERE id = ?', [userId]);
        const user = users[0];
        if (user?.invited_by) {
            const agentPricing = await db_1.agentDb.getPricing(user.invited_by);
...
async function handleConsumptionCommission(consumerId, amount, source = 'consume', pricingKey = null) {
    try {
        if (!pricingKey) {
            console.log(`ℹ️ 跳过佣金（无pricingKey）: consumerId=${consumerId} amount=${amount}`);
            return;
        }
        const consumer = await db_1.userDb.findById(consumerId);
        if (consumer && consumer.invited_by) {
            const agent = await db_1.userDb.findById(consumer.invited_by);
            if (agent && agent.is_agent) {
                const basePrice = await getPricing(pricingKey);
                const agentPricing = await db_1.agentDb.getPricing(agent.id);
                ...
                if (agentPrice && agentPrice > basePrice) {
                    ...
                    if (commissionAmount > 0) {
                        await db_1.userDb.updateCommissionBalance(agent.id, commissionAmount);
                        await db_1.commissionDb.create(agent.id, consumerId, commissionAmount, source);
...
async function getSiteConfig() {
    ...
    if (!siteConfigCache || now - siteConfigCacheTime > SITE_CONFIG_CACHE_TTL) {
        try {
            const [rows] = await db_2.pool.execute('SELECT logo_url, icon_url, site_title FROM site_config WHERE id = 1');
            if (rows && rows.length > 0) {
                siteConfigCache = {
                ...
                // 同时获取 OAuth 平台配置
                try {
                    const [oauthRows] = await db_2.pool.execute('SELECT platforms FROM oauth_config WHERE id = 1');
...
(async () => {
    ...
    `);
      ...
      `);
            const [result] = await db_2.pool.execute('SELECT id FROM users WHERE email = ?', ['softhooky@163.com']);
            if (result.length > 0) {
    ...
    try {
        await db_2.pool.execute(`ALTER TABLE generated_images ADD COLUMN task_id VARCHAR(100) NULL COMMENT '异步任务ID'`);
    ...
    try {
        await db_2.pool.execute(`ALTER TABLE generated_images MODIFY COLUMN type VARCHAR(50) NOT NULL DEFAULT 'generated'`);
    ...
    try {
        ...
        const [cols] = await db_2.pool.execute(`SHOW COLUMNS FROM generated_images LIKE 'expires_at'`);
        const col = cols[0];
        if (col && col.Extra?.includes('GENERATED')) {
            console.log('⚠️ expires_at 是 generated column，正在修改为普通列...');
            await db_2.pool.execute(`ALTER TABLE generated_images MODIFY COLUMN expires_at DATETIME NULL`);
            console.log('✅ expires_at 已修改为普通列');
        }
        // 检查 created_at 是否是 generated column
        const [caCols] = await db_2.pool.execute(`SHOW COLUMNS FROM generated_images LIKE 'created_at'`);
        const caCol = caCols[0];
        if (caCol && caCol.Extra?.includes('GENERATED')) {
            console.log('⚠️ created_at 是 generated column，正在修改为普通列...');
            await db_2.pool.execute(`ALTER TABLE generated_images MODIFY COLUMN created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP`);
    ...
    try {
        const [statusCols] = await db_2.pool.execute(`SHOW COLUMNS FROM generated_images LIKE 'status'`);
        if (statusCols.length === 0) {
            await db_2.pool.execute(`ALTER TABLE generated_images ADD COLUMN status VARCHAR(20) DEFAULT 'completed' COMMENT '任务状态: pending/completed/failed'`);
    ...
    try {
        const removedKeys = [
            ...
            'deepseek_chat', // AI文案对话
            'seedream_generation', 'seedream_img2img', // Seedream 文生图/图生图
            'agnes_video', // Agnes Video
        ];
        for (const key of removedKeys) {
            await db_2.pool.execute('DELETE FROM pricing_config WHERE `key` = ?', [key]);
            // 同时清理代理自定义定价
            await db_2.pool.execute('DELETE FROM agent_pricing WHERE pricing_key = ?', [key]);
        }
    ...
    try {
        ...
        for (const p of videoPricing) {
            await db_2.pool.execute('INSERT INTO pricing_config (`key`, name, price, enabled) VALUES (?, ?, ?, 1) ON DUPLICATE KEY UPDATE price = VALUES(price), name = VALUES(name)', [p.key, p.name, p.price]);
        }
...
(async () => {
    try {
        await db_2.pool.execute('ALTER TABLE users ADD COLUMN invited_by INT NULL');
    ...
    try {
        await db_2.pool.execute('ALTER TABLE users ADD COLUMN commission_balance DECIMAL(10,2) DEFAULT 0');
    ...
    try {
        await db_2.pool.execute('ALTER TABLE users ADD COLUMN is_agent TINYINT DEFAULT 0');
    ...
    try {
        await db_2.pool.execute('ALTER TABLE users ADD COLUMN applied_agent TINYINT DEFAULT 0');
    ...
    // 加提现账号和凭证字段
    try {
        await db_2.pool.execute('ALTER TABLE withdraw_requests ADD COLUMN account_type VARCHAR(16) NULL');
        ...
        if (error.message?.includes('Duplicate column') || error.errno === 1060) {
            ...
            console.log('ℹ️ 添加 account_type 字段失败:', error.message?.substring(0, 100));
        }
    }
    try {
        await db_2.pool.execute('ALTER TABLE withdraw_requests ADD COLUMN account_id VARCHAR(255) NULL');
        ...
        if (error.message?.includes('Duplicate column') || error.errno === 1060) {
            ...
            console.log('ℹ️ 添加 account_id 字段失败:', error.message?.substring(0, 100));
        }
    }
    try {
        await db_2.pool.execute('ALTER TABLE withdraw_requests ADD COLUMN proof_image_url TEXT NULL');
        ...
        if (error.message?.includes('Duplicate column') || error.errno === 1060) {
            ...
            console.log('ℹ️ 添加 proof_image_url 字段失败:', error.message?.substring(0, 100));
        }
    }
})();
// 初始化站点配置表
(async () => {
    ...
    `);
        await db_2.pool.execute('INSERT IGNORE INTO site_config (id, logo_url, icon_url, site_title) VALUES (1, ?, ?, ?)', ['/logo.png', '/logo.png', 'Softhooky-智能设计平台']);
...
(async () => {
    ...
    `);
        await db_2.pool.execute('INSERT IGNORE INTO oauth_config (id, qauth_api, qauth_appkey, qauth_user_secret) VALUES (1, ?, ?, ?)', ['https://api.qauth.cn', '', '']);
...
(async () => {
    ...
    `);
        ...
        for (const m of defaultModels) {
            await db_2.pool.execute('INSERT INTO models_config (model_id, label, enabled, sort_order) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE sort_order = VALUES(sort_order), label = VALUES(label)', [m.model_id, m.label, m.enabled, m.sort_order]);
        }
...
(async () => {
    ...
    `);
        ...
        for (const n of defaultNavItems) {
            await db_2.pool.execute('INSERT INTO nav_config (nav_id, label, category, enabled, sort_order) VALUES (?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE label = VALUES(label), category = VALUES(category), sort_order = VALUES(sort_order), enabled = VALUES(enabled)', [n.nav_id, n.label, n.category, n.enabled ?? 1, n.sort_order]);
        }
...
(async () => {
    ...
    `);
        ...
        console.log('ℹ️ coupons 表初始化失败:', error?.message?.substring(0, 100));
    }
    try {
        await db_2.pool.execute(`
    ...
    `);
        ...
        console.log('ℹ️ coupon_claims 表初始化失败:', error?.message?.substring(0, 100));
    }
    try {
        await db_2.pool.execute(`
    ...
    `);
        ...
        console.log('ℹ️ credit_buckets 表初始化失败:', error?.message?.substring(0, 100));
    }
    try {
        await db_2.pool.execute(`
    ...
    `);
        ...
        console.log('ℹ️ canvas_states 表初始化失败:', error?.message?.substring(0, 100));
    }
    // 创建 sessions 表（如果不存在）
    try {
        await db_2.pool.execute(`
    ...
    `);
        ...
        console.log('ℹ️ sessions 表初始化失败:', error?.message?.substring(0, 100));
    }
})();
// 初始化模型库表
(async () => {
    ...
    `);
        ...
        console.log('ℹ️ model_library 表可能已存在或创建失败:', error?.message?.substring(0, 100));
    }
})();
// 添加 thumbnail_url 列（如果不存在）
(async () => {
    try {
        const [cols] = await db_2.pool.execute(`SHOW COLUMNS FROM generated_images LIKE 'thumbnail_url'`);
        if (cols.length === 0) {
            await db_2.pool.execute(`ALTER TABLE generated_images ADD COLUMN thumbnail_url VARCHAR(500) NULL COMMENT '视频封面URL' AFTER image_url`);
            console.log('✅ generated_images 表已添加 thumbnail_url 列');
        }
    }
    catch (error) {
        console.log('ℹ️ thumbnail_url 列可能已存在:', error?.message?.substring(0, 100));
    }
})();
...
app.use(express_1.default.raw({ type: 'multipart/form-data', limit: '100mb' }));
// 验证码存储 (内存)
...
// 发送管理员通知邮件（新用户注册）
async function sendAdminNotificationEmail(email, ipAddress, userId) {
...
// 发送提现通知邮件给管理员
async function sendWithdrawNotificationEmail(agentEmail, amount, accountType, accountId) {
    const adminEmail = process.env.ADMIN_EMAIL;
    if (!adminEmail) {
        console.log('📧 未配置管理员邮箱，跳过发送通知');
        return;
...
function getCachedUsername(key) {
...
function setCachedUsername(key, username) {
    userNameCache.set(key, { username, time: Date.now() });
}
// COS 上传辅助函数
async function uploadToCos(imageBuffer, extension = 'png', userId, subUserId) {
    const now = new Date();
    ...
    if (userId) {
        try {
            ...
            if (cachedPath) {
                ...
                const user = await db_1.userDb.findById(userId);
                ...
                if (subUserId) {
                    const subUser = await db_1.userDb.findById(subUserId);
...
// COS 上传重试函数
async function uploadToCosWithRetry(imageBuffer, extension, userId, subUserId, maxRetries = 3) {
...
async function startServer() {
    ...
    app.get("/api/debug/proxy", (req, res) => {
        const isSidecar = typeof process !== 'undefined' && process.pkg;
        const fs = require('fs');
    ...
    const isSidecar = typeof process !== 'undefined' && process.pkg;
    const noEnvInCwd = !fs_1.default.existsSync('./.env');
    const proxyTarget = process.env.API_PROXY_TARGET || null;
    if (proxyTarget) {
    ...
    app.post("/api/auth/unlock", async (req, res) => {
        try {
            ...
            const session = await db_1.sessionDb.validate(token);
            if (!session)
                return res.status(401).json({ success: false, message: '登录已过期' });
            const adminUser = await db_1.userDb.findById(session.user_id);
            if (!adminUser || !adminUser.is_admin)
                return res.status(403).json({ success: false, message: '无权限，仅管理员可解锁账号' });
            const { email } = req.body;
            if (!email)
                return res.status(400).json({ success: false, message: '请输入邮箱' });
            // 重置数据库中的登录次数和锁定状态
            await db_2.pool.execute('UPDATE users SET login_attempts = 0, locked_until = NULL WHERE email = ?', [email]);
            // 清空内存中的 IP 登录失败记录
    ...
    app.post("/api/captcha/char-click/check", async (req, res) => {
        ...
        const isCorrect = clickedChars.every((c, i) => c === session.targetChars[i]);
        if (!isCorrect) {
    ...
    app.get("/api/debug/sessions", async (req, res) => {
        try {
            ...
            const [allSessions] = await db_2.pool.execute('SELECT id, user_id, token, ip_address, expires_at FROM sessions LIMIT 10');
    ...
    app.post("/api/auth/send-code", async (req, res) => {
        ...
        // 检查是否已注册
        try {
            const existingUser = await db_1.userDb.findByEmail(email);
            // 如果是重置密码，邮箱必须已注册
    ...
    app.post("/api/auth/verify-code", async (req, res) => {
        ...
        const resetToken = crypto_1.default.randomUUID().replace(/-/g, '') + crypto_1.default.randomUUID().replace(/-/g, '');
    ...
    app.post("/api/auth/reset-password", async (req, res) => {
        ...
        try {
            // 查找用户
            const user = await db_1.userDb.findByEmail(email);
            if (!user) {
                return res.status(400).json({ success: false, message: '该邮箱未注册' });
            }
            // 更新密码，同时清除登录错误次数和锁定状态
            const passwordHash = await (0, db_1.hashPassword)(password);
            await db_2.pool.execute('UPDATE users SET password_hash = ?, login_attempts = 0, locked_until = NULL WHERE id = ?', [passwordHash, user.id]);
    ...
    app.post("/api/auth/register", async (req, res) => {
        ...
        try {
            ...
            const userId = await db_1.userDb.create(email, passwordHash, finalApiKey);
            // 新注册用户赠送 1 积分
            let giftAmount = 1;
            // 处理邀请码：通过代理注册
            let invitedByAgent = null;
            if (inviteCode) {
                try {
                    // 验证邀请码
                    const inviteRecord = await db_1.inviteCodeDb.validate(inviteCode);
                    if (inviteRecord) {
                        invitedByAgent = await db_1.userDb.findById(inviteRecord.parent_user_id);
                        if (invitedByAgent && invitedByAgent.is_agent) {
                            // 绑定代理关系
                            await db_2.pool.execute('UPDATE users SET invited_by = ? WHERE id = ?', [invitedByAgent.id, userId]);
                            // 从代理的佣金余额扣除赠送积分
                            const giftCredits = parseFloat(process.env.GIFT_CREDITS_ON_INVITE || '3');
                            if (invitedByAgent.commission_balance >= giftCredits) {
                                await db_2.pool.execute('UPDATE users SET commission_balance = commission_balance - ? WHERE id = ?', [giftCredits, invitedByAgent.id]);
                                giftAmount += giftCredits;
                                await db_1.commissionDb.create(invitedByAgent.id, userId, -giftCredits, 'gift', '注册赠送');
                            }
                            else {
                                // 代理余额不足，只给基础赠送
                                giftAmount += Math.max(0, invitedByAgent.commission_balance);
                                await db_2.pool.execute('UPDATE users SET commission_balance = 0 WHERE id = ?', [invitedByAgent.id]);
                                if (invitedByAgent.commission_balance > 0) {
                                    await db_1.commissionDb.create(invitedByAgent.id, userId, -invitedByAgent.commission_balance, 'gift', '注册赠送(余额不足)');
                                }
                            }
                            // 标记邀请码已使用
                            await db_1.inviteCodeDb.markAsUsed(inviteCode);
            ...
            await db_1.userDb.updateCredits(userId, giftAmount);
            // 验证成功，删除验证码
            verificationCodes.delete(email);
            // 创建会话（传入 IP 地址，自动踢掉该账号在该 IP 的旧会话）
            const token = (0, db_1.generateToken)();
            await db_1.sessionDb.create(userId, token, ipAddress);
            // 获取用户信息
            const user = await db_1.userDb.findById(userId);
    ...
    app.post("/api/auth/login", async (req, res) => {
        ...
        try {
            ...
            // 验证密码
            const isValid = await (0, db_1.verifyPassword)(password, user.password_hash);
            if (!isValid) {
                // 密码错误，增加登录失败次数
                ...
                if (newAttempts >= 5) {
                    ...
                    await db_2.pool.execute('UPDATE users SET login_attempts = ?, locked_until = ? WHERE id = ?', [newAttempts, tomorrow, user.id]);
                ...
                await db_2.pool.execute('UPDATE users SET login_attempts = ? WHERE id = ?', [newAttempts, user.id]);
            ...
            // 登录成功，清除登录失败次数和锁定状态
            await db_2.pool.execute('UPDATE users SET login_attempts = 0, locked_until = NULL WHERE id = ?', [user.id]);
            // 清除 IP 登录失败记录
            clearIPLoginFailure(ipAddress);
            // 更新最后登录时间
            await db_1.userDb.updateLastLogin(user.id);
            // 创建会话（传入 IP 地址，自动踢掉该账号在该 IP 的旧会话）
            const token = (0, db_1.generateToken)();
            console.log(`🔐 用户登录: ${email} (IP: ${ipAddress}), 进程ID: ${process.pid}`);
            await db_1.sessionDb.create(user.id, token, ipAddress);
            console.log('✅ 会话创建成功，已踢掉该账号在该 IP 的旧会话');
            // 验证session是否创建成功
            const [verifyRows] = await db_2.pool.execute('SELECT id, token, LEFT(token, 10) as prefix, created_at FROM sessions WHERE user_id = ? ORDER BY id DESC', [user.id]);
            res.json({
                success: true,
                token,
                user: {
    ...
    const adminMiddleware = async (req, res, next) => {
        ...
        const user = await db_1.userDb.findById(session.user_id);
    ...
    app.post("/api/admin/login", async (req, res) => {
        try {
            ...
            // 从数据库查询用户
            const [users] = await db_2.pool.execute('SELECT * FROM users WHERE email = ?', [email]);
    ...
    app.get("/api/admin/dashboard", adminMiddleware, async (req, res) => {
        try {
            // 总用户数
            const [userCountResult] = await db_2.pool.execute('SELECT COUNT(*) as count FROM users');
            const totalUsers = userCountResult[0].count;
            // 今日新用户
            const [todayUsersResult] = await db_2.pool.execute("SELECT COUNT(*) as count FROM users WHERE DATE(created_at) = CURDATE()");
            const todayNewUsers = todayUsersResult[0].count;
            // 总充值金额
            const [totalRechargeResult] = await db_2.pool.execute("SELECT COALESCE(SUM(amount), 0) as total FROM payment_orders WHERE status = 'completed'");
            const totalRechargeAmount = parseFloat(totalRechargeResult[0].total) || 0;
            // 今日充值金额
            const [todayRechargeResult] = await db_2.pool.execute("SELECT COALESCE(SUM(amount), 0) as total FROM payment_orders WHERE status = 'completed' AND DATE(created_at) = CURDATE()");
            const todayRechargeAmount = parseFloat(todayRechargeResult[0].total) || 0;
            // 待处理订单
            const [pendingOrdersResult] = await db_2.pool.execute("SELECT COUNT(*) as count FROM payment_orders WHERE status = 'pending'");
            const pendingOrders = pendingOrdersResult[0].count;
            // 总消费积分
            const [consumptionResult] = await db_2.pool.execute("SELECT COALESCE(SUM(ABS(amount)), 0) as total FROM credit_transactions WHERE amount < 0");
            const totalConsumption = parseFloat(consumptionResult[0].total) || 0;
            // 今日消费积分
            const [todayConsumptionResult] = await db_2.pool.execute("SELECT COALESCE(SUM(ABS(amount)), 0) as total FROM credit_transactions WHERE amount < 0 AND DATE(created_at) = CURDATE()");
            const todayConsumption = parseFloat(todayConsumptionResult[0].total) || 0;
            // 近7天每日充值金额
            const [dailyRechargeResult] = await db_2.pool.execute(`
      ...
      `);
            // 近7天每日消费积分
            const [dailyConsumptionResult] = await db_2.pool.execute(`
    ...
    app.get("/api/admin/users", adminMiddleware, async (req, res) => {
        try {
            ...
            const [totalResult] = await db_2.pool.execute('SELECT COUNT(*) as total FROM users');
            const total = totalResult[0].total;
            const [users] = await db_2.pool.execute(`SELECT u.id, u.email, u.credits, u.is_enabled, u.recharge_disabled, u.is_admin, u.is_agent, u.applied_agent, u.created_at, u.last_login_at, u.invited_by, inviter.email as inviter_email
         FROM users u LEFT JOIN users inviter ON u.invited_by = inviter.id
         ORDER BY u.created_at DESC LIMIT ? OFFSET ?`, [pageSize, offset]);
            res.json({
                success: true,
                data: users,
    ...
    app.post("/api/admin/users/:id/toggle", adminMiddleware, async (req, res) => {
        try {
            ...
            await db_2.pool.execute('UPDATE users SET is_enabled = ? WHERE id = ?', [isEnabled ? 1 : 0, userId]);
    ...
    // 禁止/允许用户充值
    app.post("/api/admin/users/:id/toggle-recharge", adminMiddleware, async (req, res) => {
        try {
            const userId = parseInt(req.params.id);
            ...
            await db_2.pool.execute('UPDATE users SET recharge_disabled = ? WHERE id = ?', [rechargeDisabled ? 1 : 0, userId]);
    ...
    app.get("/api/admin/users/:id", adminMiddleware, async (req, res) => {
        try {
            const userId = parseInt(req.params.id);
            const rechargePage = Math.max(1, parseInt(req.query.rechargePage) || 1);
            const rechargePageSize = 10;
            const rechargeOffset = (rechargePage - 1) * rechargePageSize;
            const [userResult] = await db_2.pool.execute(`SELECT u.id, u.email, u.credits, u.is_enabled, u.recharge_disabled, u.created_at, u.last_login_at, u.invited_by, inviter.email as inviter_email
         FROM users u LEFT JOIN users inviter ON u.invited_by = inviter.id
         WHERE u.id = ?`, [userId]);
            ...
            // 充值记录分页
            const [recharges] = await db_2.pool.execute('SELECT id, order_id, amount, status, created_at FROM payment_orders WHERE user_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?', [userId, rechargePageSize, rechargeOffset]);
            const [rechargeCountResult] = await db_2.pool.execute('SELECT COUNT(*) as total FROM payment_orders WHERE user_id = ?', [userId]);
            const rechargeTotal = rechargeCountResult[0].total;
            const [consumptions] = await db_2.pool.execute(`SELECT id, amount, type, description, created_at FROM credit_transactions WHERE user_id = ? AND type = 'consumption' ORDER BY created_at DESC LIMIT 20`, [userId]);
            const [subUsers] = await db_2.pool.execute('SELECT id, email, name, is_enabled, created_at FROM sub_users WHERE parent_user_id = ?', [userId]);
            res.json({
                success: true,
                user,
                recharges,
    ...
    app.get("/api/admin/orders", adminMiddleware, async (req, res) => {
        try {
            ...
            const [orders] = await db_2.pool.query(query, params);
            const [totalResult] = await db_2.pool.query(countQuery, countParams);
            const total = totalResult[0].total;
            res.json({
                success: true,
                data: orders,
    ...
    app.put("/api/admin/orders/:id", adminMiddleware, async (req, res) => {
        try {
            ...
            await db_2.pool.execute('UPDATE payment_orders SET status = ? WHERE id = ?', [status, orderId]);
    ...
    app.get("/api/admin/notifications", adminMiddleware, async (req, res) => {
        try {
            ...
            const pageSize = 20;
            const offset = (page - 1) * pageSize;
            const [totalResult] = await db_2.pool.execute('SELECT COUNT(*) as total FROM notifications');
            const total = totalResult[0].total;
            const [notifications] = await db_2.pool.execute('SELECT * FROM notifications ORDER BY created_at DESC LIMIT ? OFFSET ?', [pageSize, offset]);
            res.json({
                success: true,
                data: notifications,
                pagination: { page, pageSize, total, totalPages: Math.ceil(total / pageSize) }
    ...
    app.post("/api/admin/notifications", adminMiddleware, async (req, res) => {
        try {
            ...
            await db_2.pool.execute('INSERT INTO notifications (title, content, is_active, created_at) VALUES (?, ?, ?, NOW())', [title, content, is_active !== false]);
    ...
    // 管理后台更新通知
    app.put("/api/admin/notifications/:id", adminMiddleware, async (req, res) => {
        try {
            const id = parseInt(req.params.id);
            const { title, content, is_active } = req.body;
            await db_2.pool.execute('UPDATE notifications SET title = ?, content = ?, is_active = ? WHERE id = ?', [title, content, is_active !== false, id]);
    ...
    app.delete("/api/admin/notifications/:id", adminMiddleware, async (req, res) => {
        try {
            const id = parseInt(req.params.id);
            await db_2.pool.execute('DELETE FROM notifications WHERE id = ?', [id]);
    ...
    app.get("/api/notifications", async (req, res) => {
        try {
            const [notifications] = await db_2.pool.execute('SELECT * FROM notifications WHERE is_active = 1 ORDER BY created_at DESC');
    ...
    app.get("/api/pricing", async (req, res) => {
        try {
            // 获取平台默认价格
            const [rows] = await db_2.pool.execute('SELECT `key`, name, price, enabled FROM pricing_config');
            const pricing = {};
    ...
    app.get("/api/models", async (req, res) => {
        try {
            const [rows] = await db_2.pool.execute('SELECT model_id, label, enabled, sort_order FROM models_config WHERE enabled = 1 ORDER BY sort_order ASC');
            if (rows.length > 0) {
    ...
    // 获取模型列表（管理员，含禁用）
    app.get("/api/admin/models", adminMiddleware, async (req, res) => {
        try {
            const [rows] = await db_2.pool.execute('SELECT model_id, label, enabled, sort_order FROM models_config ORDER BY sort_order ASC');
            res.json({ success: true, data: rows });
        }
        catch (error) {
    ...
    // 切换模型启用/禁用
    app.put("/api/admin/models/:modelId/toggle", adminMiddleware, async (req, res) => {
        try {
            const { modelId } = req.params;
            const [rows] = await db_2.pool.execute('SELECT enabled FROM models_config WHERE model_id = ?', [modelId]);
            if (rows.length === 0)
                return res.status(404).json({ success: false, message: '模型不存在' });
            const newEnabled = rows[0].enabled ? 0 : 1;
            await db_2.pool.execute('UPDATE models_config SET enabled = ? WHERE model_id = ?', [newEnabled, modelId]);
    ...
    app.put("/api/admin/models/:modelId", adminMiddleware, async (req, res) => {
        try {
            const { modelId } = req.params;
            const { label, sort_order } = req.body;
            if (label)
                await db_2.pool.execute('UPDATE models_config SET label = ? WHERE model_id = ?', [label, modelId]);
            if (sort_order !== undefined)
                await db_2.pool.execute('UPDATE models_config SET sort_order = ? WHERE model_id = ?', [sort_order, modelId]);
    ...
    app.get("/api/nav", async (req, res) => {
        try {
            // 自动同步前端新增的导航项到数据库
            // 清理已下架的旧导航项
            await db_2.pool.execute("DELETE FROM nav_config WHERE nav_id IN ('nano-gen', 'detail', 'sora2-video', 'styleCopy', 'gpt54-chat', 'tryon', 'carousel', 'deepseek-chat')");
            ...
            for (const item of REQUIRED_NAV_ITEMS) {
                const [existing] = await db_2.pool.execute('SELECT nav_id FROM nav_config WHERE nav_id = ?', [item.nav_id]);
                if (existing.length === 0) {
                    await db_2.pool.execute('INSERT INTO nav_config (nav_id, label, category, enabled, sort_order) VALUES (?, ?, ?, ?, ?)', [item.nav_id, item.label, item.category, item.enabled, item.sort_order]);
                }
            }
            const [rows] = await db_2.pool.execute('SELECT nav_id, label, category, enabled, sort_order FROM nav_config WHERE enabled = 1 ORDER BY sort_order ASC');
            res.json({ success: true, data: rows });
        }
        catch (error) {
    ...
    // 获取导航菜单列表（管理员，含禁用）
    app.get("/api/admin/nav", adminMiddleware, async (req, res) => {
        try {
            // 清理已删除的旧导航项
            await db_2.pool.execute("DELETE FROM nav_config WHERE nav_id IN ('nano-gen', 'detail', 'sora2-video', 'styleCopy', 'gpt54-chat', 'tryon', 'carousel', 'deepseek-chat')");
            const [rows] = await db_2.pool.execute('SELECT nav_id, label, category, enabled, sort_order FROM nav_config ORDER BY sort_order ASC');
            res.json({ success: true, data: rows });
        }
        catch (error) {
            res.status(500).json({ success: false, message: '获取导航菜单失败' });
    ...
    // 切换导航菜单启用/禁用
    app.put("/api/admin/nav/:navId/toggle", adminMiddleware, async (req, res) => {
        try {
            const { navId } = req.params;
            const [rows] = await db_2.pool.execute('SELECT enabled FROM nav_config WHERE nav_id = ?', [navId]);
            if (rows.length === 0)
                return res.status(404).json({ success: false, message: '导航项不存在' });
            const newEnabled = rows[0].enabled ? 0 : 1;
            await db_2.pool.execute('UPDATE nav_config SET enabled = ? WHERE nav_id = ?', [newEnabled, navId]);
            res.json({ success: true, enabled: !!newEnabled });
        }
        catch (error) {
            res.status(500).json({ success: false, message: '切换失败' });
    ...
    app.put("/api/admin/nav/:navId", adminMiddleware, async (req, res) => {
        try {
            const { navId } = req.params;
            const { label, sort_order } = req.body;
            if (label)
                await db_2.pool.execute('UPDATE nav_config SET label = ? WHERE nav_id = ?', [label, navId]);
            if (sort_order !== undefined)
                await db_2.pool.execute('UPDATE nav_config SET sort_order = ? WHERE nav_id = ?', [sort_order, navId]);
            res.json({ success: true });
        }
        catch (error) {
            res.status(500).json({ success: false, message: '更新失败' });
    ...
    const authMiddleware = async (req, res, next) => {
        ...
        if (!session) {
            ...
            try {
                const [rows] = await db_2.pool.execute('SELECT * FROM sessions WHERE token = ?', [token]);
        ...
        if (session.sub_user_id) {
            // 子账号登录
            const subUser = await db_1.subUserDb.findById(session.sub_user_id);
            const parentUser = await db_1.userDb.findById(session.user_id);
    ...
    app.get("/api/auth/me", authMiddleware, async (req, res) => {
        try {
            ...
            if (!isEnabled) {
                ...
                if (req.user.isSubUser && req.user.parentUserId) {
                    const parentUser = await db_1.userDb.findById(req.user.parentUserId);
                    parentEmail = parentUser?.email || null;
                }
                return res.status(403).json({
                    success: false,
    ...
    // 刷新用户积分 - 每次自动同步
    app.get("/api/auth/credits", authMiddleware, async (req, res) => {
        try {
            let userId = req.user.id;
            let isEnabled = true;
            // 如果是子账号，获取主账号的积分
            ...
            // 清理过期优惠券积分
            try {
                await db_1.creditBucketDb.cleanExpired(userId);
            }
            catch (e) {
                // 清理失败不影响主流程
            }
            // 重新获取清理后的余额
            const cleanedUser = await db_1.userDb.findById(userId);
    ...
    function getHash(params, appSecret) {
        ...
        return crypto_1.default.createHash('md5').update(stringSignTemp).digest('hex');
    }
    // 生成UUID
    ...
    app.post("/api/payment/initiate", authMiddleware, async (req, res) => {
        try {
            ...
            // 检查用户是否被禁止充值
            const payingUser = await db_1.userDb.findById(req.user.isSubUser ? req.user.parentUserId : req.user.id);
            ...
            let paymentUrl = null;
            try {
                const response = await axios_1.default.post(PAYMENT_CONFIG.PAYMENT_API_URL, requestParams, {
                    headers: {
            ...
            await db_2.pool.execute('INSERT INTO payment_orders (user_id, order_id, amount, status, created_at) VALUES (?, ?, ?, ?, NOW())', [userId, orderId, amount, 'pending']);
            console.log('✅ 订单已创建，等待支付回调:', { orderId, userId, amount });
            // 如果支付网关失败，返回错误
            if (!paymentUrl) {
                console.error('❌ 支付网关未返回支付URL');
                // 更新订单状态为失败
                await db_2.pool.execute('UPDATE payment_orders SET status = ? WHERE order_id = ?', ['failed', orderId]);
    ...
    // 测试 POST 回调
    app.post("/api/payment/test-notify", express_1.default.urlencoded({ extended: true }), (req, res) => {
        console.log('🔔 测试 POST 回调被访问');
        console.log('📥 收到数据:', req.body);
        res.send('success');
    });
    // 手动处理 pending 订单（用于调试）
    app.post("/api/payment/manual-process", authMiddleware, async (req, res) => {
        try {
            const userId = req.user.isSubUser ? req.user.parentUserId : req.user.id;
            console.log('🔧 手动处理 pending 订单，用户ID:', userId);
            const [pendingOrders] = await db_2.pool.execute(`SELECT * FROM payment_orders
         ...
         ORDER BY created_at DESC LIMIT 10`, [userId]);
            ...
            for (const order of pendingOrders) {
                ...
                await db_2.pool.execute('UPDATE users SET credits = ? WHERE id = ?', [newCredits, userId]);
                await db_2.pool.execute('INSERT INTO credit_buckets (user_id, total_amount, remaining_amount, source, expires_at) VALUES (?, ?, ?, ?, NULL)', [userId, amount, amount, 'recharge']);
                await db_2.pool.execute('INSERT INTO credit_transactions (user_id, amount, type, description, created_at) VALUES (?, ?, ?, ?, NOW())', [userId, amount, 'recharge', `手动处理 - 订单号: ${order.order_id}`]);
                await db_2.pool.execute('UPDATE payment_orders SET status = ?, updated_at = NOW() WHERE order_id = ?', ['completed', order.order_id]);
                processed++;
    ...
    // 检查支付状态（用于前端轮询）
    app.post("/api/payment/check-status", authMiddleware, async (req, res) => {
        try {
            const { startTime } = req.body;
            const userId = req.user.isSubUser ? req.user.parentUserId : req.user.id;
            // 查询该用户最新的已完成订单
            const [orders] = await db_2.pool.execute('SELECT * FROM payment_orders WHERE user_id = ? AND status = "completed" ORDER BY created_at DESC LIMIT 1', [userId]);
    ...
    // 查询订单状态（用于前端轮询）
    app.get("/api/payment/order/:orderId", authMiddleware, async (req, res) => {
        try {
            const orderId = req.params.orderId;
            const userId = req.user.isSubUser ? req.user.parentUserId : req.user.id;
            const [orders] = await db_2.pool.execute('SELECT * FROM payment_orders WHERE order_id = ? AND user_id = ?', [orderId, userId]);
            if (!orders || orders.length === 0) {
    ...
    // 支付回调处理
    app.post("/api/payment/notify", express_1.default.urlencoded({ extended: true }), async (req, res) => {
        try {
            const data = req.body;
            console.log('📥 收到支付回调 ==================');
            ...
            // 查询订单信息（所有状态的都查出来）
            const [allOrders] = await db_2.pool.execute('SELECT * FROM payment_orders WHERE order_id = ?', [orderId]);
            ...
            await db_2.pool.execute('INSERT INTO credit_buckets (user_id, total_amount, remaining_amount, source, expires_at) VALUES (?, ?, ?, ?, NULL)', [userId, amount, amount, 'recharge']);
            // 验证积分是否更新成功
            const [verifyUser] = await db_2.pool.execute('SELECT credits FROM users WHERE id = ?', [userId]);
            const updatedCredits = verifyUser[0]?.credits;
            console.log('✅ 积分更新验证:', updatedCredits);
            // 记录交易
            await db_2.pool.execute('INSERT INTO credit_transactions (user_id, amount, type, description, created_at) VALUES (?, ?, ?, ?, NOW())', [userId, amount, 'recharge', `${paymentMethod}充值${signatureValid ? '' : '(验签失败)'} - 订单号: ${orderId}`]);
            // 更新订单状态为 completed
            await db_2.pool.execute('UPDATE payment_orders SET status = ?, updated_at = NOW() WHERE order_id = ?', ['completed', orderId]);
            console.log('✅ 充值成功:', {
                userId,
                amount,
    ...
    // 充值积分
    app.post("/api/auth/recharge", authMiddleware, async (req, res) => {
        ...
        try {
            await db_1.creditTransactionDb.create(userId, amount, 'recharge', '积分充值');
            await db_1.userDb.updateCredits(userId, (await db_1.userDb.findById(userId)).credits + amount);
            await db_2.pool.execute('INSERT INTO credit_buckets (user_id, total_amount, remaining_amount, source, expires_at) VALUES (?, ?, ?, ?, NULL)', [userId, amount, amount, 'recharge']);
            // 充值佣金：充值不涉及具体服务，佣金在消费时按代理差价计算
            // 此处不再发放充值佣金
            const updatedUser = await db_1.userDb.findById(userId);
            res.json({
                success: true,
    ...
    app.post("/api/payment/sync-credits", authMiddleware, async (req, res) => {
        try {
            const userId = req.user.isSubUser ? req.user.parentUserId : req.user.id;
            console.log('🔧 sync-credits 调用，用户ID:', userId);
            // 查询已完成的订单（24小时内）
            const [ordersResult] = await db_2.pool.execute(`SELECT SUM(CAST(amount AS DECIMAL(10,2))) as total FROM payment_orders
         WHERE user_id = ? AND status = "completed"
         AND created_at > DATE_SUB(NOW(), INTERVAL 24 HOUR)`, [userId]);
            const ordersTotal = parseFloat(ordersResult[0]?.total?.toString() || '0');
            // 查询已同步的交易（24小时内）
            const [transResult] = await db_2.pool.execute(`SELECT SUM(CAST(amount AS DECIMAL(10,2))) as total FROM credit_transactions
         WHERE user_id = ? AND type = "recharge"
         AND created_at > DATE_SUB(NOW(), INTERVAL 24 HOUR)`, [userId]);
            const transTotal = parseFloat(transResult[0]?.total?.toString() || '0');
            // 计算差异并补录
            const diff = ordersTotal - transTotal;
            console.log('📋 订单总额:', ordersTotal, '交易总额:', transTotal, '差异:', diff);
            if (diff > 0.01) {
                const [userResult] = await db_2.pool.execute('SELECT credits FROM users WHERE id = ?', [userId]);
                const currentCredits = parseFloat(userResult[0]?.credits?.toString() || '0');
                const newCredits = currentCredits + diff;
                await db_2.pool.execute('INSERT INTO credit_transactions (user_id, amount, type, description, created_at) VALUES (?, ?, ?, ?, NOW())', [userId, diff, 'recharge', `积分同步补录 - 金额: ${diff}`]);
                await db_2.pool.execute('UPDATE users SET credits = ? WHERE id = ?', [newCredits, userId]);
                await db_2.pool.execute('INSERT INTO credit_buckets (user_id, total_amount, remaining_amount, source, expires_at) VALUES (?, ?, ?, ?, NULL)', [userId, diff, diff, 'recharge']);
    ...
    // 手动完成订单（用于测试或手动处理支付）
    app.post("/api/payment/manual-complete", authMiddleware, async (req, res) => {
        try {
            ...
            await db_2.pool.execute('INSERT INTO credit_buckets (user_id, total_amount, remaining_amount, source, expires_at) VALUES (?, ?, ?, ?, NULL)', [userId, amount, amount, 'recharge']);
            // 创建交易记录
            await db_2.pool.execute('INSERT INTO credit_transactions (user_id, amount, type, description, created_at) VALUES (?, ?, ?, ?, NOW())', [userId, amount, 'recharge', `手动完成充值 - 订单号: ${orderId}`]);
            // 更新订单状态
            await db_2.pool.execute('UPDATE payment_orders SET status = ?, updated_at = NOW() WHERE order_id = ?', ['completed', orderId]);
    ...
    app.post("/api/payment/fix-pending", authMiddleware, async (req, res) => {
        try {
            const userId = req.user.isSubUser ? req.user.parentUserId : req.user.id;
         ...
         ORDER BY created_at DESC`, [userId]);
            ...
            for (const order of orders) {
                ...
                // 创建交易记录
                await db_2.pool.execute('INSERT INTO credit_transactions (user_id, amount, type, description, created_at) VALUES (?, ?, ?, ?, NOW())', [userId, amount, 'recharge', `自动修复充值 - 订单号: ${order.order_id}`]);
                // 更新订单状态
                await db_2.pool.execute('UPDATE payment_orders SET status = ?, updated_at = NOW() WHERE order_id = ?', ['completed', order.order_id]);
                fixedCount++;
            }
            ...
            if (totalAmount > 0) {
                ...
                await db_2.pool.execute('UPDATE users SET credits = ? WHERE id = ?', [newCredits, userId]);
                await db_2.pool.execute('INSERT INTO credit_buckets (user_id, total_amount, remaining_amount, source, expires_at) VALUES (?, ?, ?, ?, NULL)', [userId, totalAmount, totalAmount, 'recharge']);
                // 验证更新
                const [verifyResult] = await db_2.pool.execute('SELECT credits FROM users WHERE id = ?', [userId]);
                console.log('✅ 验证积分:', verifyResult[0]?.credits);
            }
            res.json({
                success: true,
    ...
    app.post("/api/payment/confirm", authMiddleware, async (req, res) => {
        try {
            ...
            if (order.status === 'pending') {
                ...
                const newCredits = currentCredits + amount;
                // 更新用户积分
                await db_1.userDb.updateCredits(userId, newCredits);
                await db_2.pool.execute('INSERT INTO credit_buckets (user_id, total_amount, remaining_amount, source, expires_at) VALUES (?, ?, ?, ?, NULL)', [userId, amount, amount, 'recharge']);
                // 记录交易
                await db_1.creditTransactionDb.create(userId, amount, 'recharge', `支付充值 - 订单号: ${orderId}`);
                // 更新订单状态
                await db_2.pool.execute('UPDATE payment_orders SET status = ? WHERE order_id = ?', ['completed', orderId]);
    ...
    app.post("/api/payment/fix-pending", authMiddleware, async (req, res) => {
        try {
            ...
            // 查询该用户所有待处理的订单
            const [pendingOrders] = await db_2.pool.execute('SELECT * FROM payment_orders WHERE user_id = ? AND status = ?', [userId, 'pending']);
            console.log('📋 查询到待处理订单:', pendingOrders?.length || 0);
            if (!pendingOrders || pendingOrders.length === 0) {
            ...
            for (const order of pendingOrders) {
                ...
                console.log(`📝 处理订单 ${order.order_id}，金额: ¥${amount}`);
                // 更新订单状态
                await db_2.pool.execute('UPDATE payment_orders SET status = ? WHERE order_id = ?', ['completed', order.order_id]);
                // 记录交易
                await db_1.creditTransactionDb.create(userId, amount, 'recharge', `支付充值 - 订单号: ${order.order_id}`);
            }
            // 一次性更新用户积分
            const newCredits = currentCredits + totalAmount;
            console.log(`💾 更新积分: ${currentCredits} + ${totalAmount} = ${newCredits}`);
            ...
            console.log('✅ 修复完成:', {
                userId,
                fixedCount: pendingOrders.length,
                totalAmount,
                previousCredits: currentCredits,
                newCredits,
    ...
    // 直接修复所有待处理订单（管理员用）
    app.post("/api/payment/admin-fix-all", authMiddleware, async (req, res) => {
        try {
            const userId = req.user.id;
            // 获取该用户的所有待处理订单
            const [pendingOrders] = await db_2.pool.execute('SELECT * FROM payment_orders WHERE user_id = ? AND status = "pending"', [userId]);
            if (!pendingOrders || pendingOrders.length === 0) {
            ...
            // 更新所有订单状态为已完成
            for (const order of orders) {
                await db_2.pool.execute('UPDATE payment_orders SET status = "completed" WHERE order_id = ?', [order.order_id]);
            ...
            // 为每个订单记录交易
            for (const order of orders) {
                await db_1.creditTransactionDb.create(userId, order.amount, 'recharge', `支付充值 - 订单号: ${order.order_id}`);
            }
            console.log('✅ 管理员修复完成:', {
                userId,
    ...
    // 手动更新特定订单状态（管理员用）
    app.post("/api/payment/admin-update-order", authMiddleware, async (req, res) => {
        try {
            ...
            // 更新订单状态
            await db_2.pool.execute('UPDATE payment_orders SET status = ? WHERE order_id = ?', [status, orderId]);
            res.json({
                success: true,
                message: '订单已更新',
                orderId,
    ...
    app.post("/api/payment/sync-credits", authMiddleware, async (req, res) => {
        try {
            const userId = req.user.id;
            // 查询所有已完成的订单
            const [completedOrders] = await db_2.pool.execute('SELECT * FROM payment_orders WHERE user_id = ? AND status = "completed" ORDER BY created_at ASC', [userId]);
            if (!completedOrders || completedOrders.length === 0) {
                return res.json({ success: true, message: '没有已完成的订单', synced: 0, totalAmount: 0 });
            }
            // 查询已记录的交易
            const [transactions] = await db_2.pool.execute('SELECT description FROM credit_transactions WHERE user_id = ? AND type = "recharge"', [userId]);
            const transactionDescriptions = new Set(transactions.map(t => t.description));
            let totalAmount = 0;
            const user = await db_1.userDb.findById(userId);
            ...
            for (const order of completedOrders) {
                ...
                if (!transactionDescriptions.has(transactionDesc)) {
                    ...
                    syncedCount++;
                    // 记录交易
                    await db_1.creditTransactionDb.create(userId, amount, 'recharge', transactionDesc);
                }
            }
            // 一次性更新用户积分
            if (syncedCount > 0) {
    ...
    app.get("/api/payment/records", authMiddleware, async (req, res) => {
        try {
            ...
            const offset = (page - 1) * pageSize;
            // 顺序执行查询
            const ordersResult = await db_2.pool.execute('SELECT id, order_id, amount, status, created_at FROM payment_orders WHERE user_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?', [userId, pageSize, offset]);
            const countResult = await db_2.pool.execute('SELECT COUNT(*) as total FROM payment_orders WHERE user_id = ?', [userId]);
            const statsResult = await db_2.pool.execute('SELECT SUM(amount) as total_amount, COUNT(*) as total_count FROM payment_orders WHERE user_id = ? AND status = "completed"', [userId]);
            console.log('=== DEBUG ===');
            ...
            const totalOrders = Array.isArray(countResult[0]) && countResult[0].length > 0 ? countResult[0][0].total : 0;
            const stats = Array.isArray(statsResult[0]) && statsResult[0].length > 0 ? statsResult[0][0] : { total_amount: 0, total_count: 0 };
            res.json({
                success: true,
                orders: ordersData || [],
                pagination: {
    ...
    // 获取消费记录
    app.get("/api/payment/consumption", authMiddleware, async (req, res) => {
        try {
            let userId = req.user.id;
            let isSubUser = false;
            if (req.user.isSubUser && req.user.parentUserId) {
            ...
            const pageSize = 10;
            const offset = (page - 1) * pageSize;
            const userResult = await db_2.pool.execute('SELECT credits, sub_quota_mode FROM users WHERE id = ?', [userId]);
            ...
            const getCreditAmount = (type, amount) => {
                if (amount > 0)
                    return amount;
            ...
            const consumptionsResult = await db_2.pool.execute(query, queryParams);
            const countResult = await db_2.pool.execute(countQuery, countParams);
            let allQuery = '';
            if (isSubUser) {
            ...
            const allRecordsResult = await db_2.pool.execute(allQuery, isSubUser ? [req.user.id] : [userId, userId]);
            const sortedRecords = (allRecordsResult[0] || []).sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime());
            const totalAllAmount = sortedRecords.reduce((sum, r) => sum + parseFloat(r.amount || '0'), 0);
            ...
            const consumptionsData = (consumptionsResult[0] || []).map((c) => {
            ...
            const totalConsumptions = Array.isArray(countResult[0]) && countResult[0].length > 0 ? countResult[0][0].total : 0;
            res.json({
                success: true,
    ...
    // 清理虚拟支付数据（仅保留真实充值）
    app.post("/api/payment/cleanup", authMiddleware, async (req, res) => {
        try {
            // 只有主账号可以清理数据
            if (req.user.isSubUser) {
            ...
            const [deleteOrdersResult] = await db_2.pool.execute('DELETE FROM payment_orders WHERE user_id = ? AND status = "pending"', [userId]);
            const deletedOrders = deleteOrdersResult.affectedRows || 0;
            // 2. 获取所有虚拟交易（除了 ¥1.00 的）
            const [virtualTransactions] = await db_2.pool.execute('SELECT id FROM credit_transactions WHERE user_id = ? AND type = "recharge" AND amount != 1.00', [userId]);
            const virtualIds = virtualTransactions.map(t => t.id);
            let deletedTransactions = 0;
            if (virtualIds.length > 0) {
                const placeholders = virtualIds.map(() => '?').join(',');
                const [deleteTransResult] = await db_2.pool.execute(`DELETE FROM credit_transactions WHERE id IN (${placeholders})`, virtualIds);
                deletedTransactions = deleteTransResult.affectedRows || 0;
            }
            // 3. 计算真实的积分（只有 ¥1.00 的充值）
            const [realTransactions] = await db_2.pool.execute('SELECT SUM(amount) as total FROM credit_transactions WHERE user_id = ? AND type = "recharge"', [userId]);
            const realCredits = realTransactions[0]?.total || 0;
            // 4. 更新用户积分
            await db_1.userDb.updateCredits(userId, realCredits);
    ...
    // 恢复充值数据（如果清理出错）
    app.post("/api/payment/restore", authMiddleware, async (req, res) => {
        try {
            // 只有主账号可以恢复数据
            if (req.user.isSubUser) {
                return res.status(403).json({ success: false, message: '子账号无权限' });
            ...
            const userId = req.user.id;
            // 检查是否已有 ¥1.00 的充值
            const [existing] = await db_2.pool.execute('SELECT id FROM credit_transactions WHERE user_id = ? AND type = "recharge" AND amount = 1.00', [userId]);
            if (existing.length > 0) {
                return res.status(400).json({ success: false, message: '已存在 ¥1.00 的充值记录' });
            }
            // 恢复 ¥1.00 的充值
            await db_1.creditTransactionDb.create(userId, 1.00, 'recharge', '微信支付充值 - 订单号: order_restored');
            await db_1.userDb.updateCredits(userId, 1.00);
    ...
    // 反推提示词扣费
    app.post("/api/prompt-reverse/deduct-credits", authMiddleware, async (req, res) => {
        try {
            ...
            const deductResult = await db_1.creditTransactionDb.deduct(parentUserId, totalCredits, 'prompt_reverse', `反推提示词 - ${imageCount}张图片${req.user.isSubUser ? '(子账号)' : ''}`, parentUserId, req.user.isSubUser ? userId : undefined);
    ...
    app.post("/api/auth/logout", authMiddleware, async (req, res) => {
        const token = req.headers.authorization?.replace('Bearer ', '');
        if (token) {
            await db_1.sessionDb.delete(token);
        }
        res.json({ success: true, message: '已退出登录' });
    });
    // 更新用户 API Key（管理员用）
    app.post("/api/auth/update-api-key", authMiddleware, async (req, res) => {
        try {
            const { email, apiKey } = req.body;
            if (!email || !apiKey) {
                return res.status(400).json({ error: '邮箱和 API Key 不能为空' });
            }
            await db_1.userDb.updateApiKey(email, apiKey);
    ...
    function oauthCallbackHtml(errorMsg, token, user) {
        if (errorMsg) {
    ...
    // OAuth 回调处理核心逻辑
    async function handleOAuthCallback(code, state, req, res) {
        ...
        if (global.__oauth_states)
            global.__oauth_states.delete(state);
        // 获取 OAuth 配置
        const [configRows] = await db_2.pool.execute('SELECT * FROM oauth_config WHERE id = 1');
        ...
        const qauthResponse = await axios_1.default.get(`${qauth_api}/authinfov2`, {
        ...
        if (unionId) {
            const [unionRows] = await db_2.pool.execute('SELECT user_id FROM user_oauth_links WHERE provider = ? AND union_id = ? LIMIT 1', ['wechat', unionId]);
        ...
        // 再查 provider_uid
        if (!existingUser) {
            const [uidRows] = await db_2.pool.execute('SELECT user_id FROM user_oauth_links WHERE provider = ? AND provider_uid = ? LIMIT 1', [provider, providerUid]);
        ...
        if (existingUser) {
            ...
            const oauthApiKey = 'sk-' + crypto_1.default.randomBytes(32).toString('hex');
            const [insertResult] = await db_2.pool.execute('INSERT INTO users (email, password_hash, credits, is_enabled, api_key) VALUES (?, ?, ?, 1, ?)', [email, await (0, db_1.hashPassword)(randomPassword), 1, oauthApiKey]);
            finalUserId = insertResult.insertId;
            await db_2.pool.execute('INSERT INTO user_oauth_links (user_id, provider, provider_uid, detail_type, union_id) VALUES (?, ?, ?, ?, ?)', [finalUserId, provider, providerUid, detailType, unionId]);
        }
        // 生成 token 登录
        const token = (0, db_1.generateToken)();
        await db_1.sessionDb.create(finalUserId, token);
        await db_1.userDb.updateLastLogin(finalUserId);
        // 获取用户信息
        const [userRows] = await db_2.pool.execute('SELECT * FROM users WHERE id = ?', [finalUserId]);
    ...
    app.get("/api/auth/oauth-config", async (req, res) => {
        try {
            const [rows] = await db_2.pool.execute('SELECT qauth_api, platforms FROM oauth_config WHERE id = 1');
            if (rows && rows.length > 0) {
    ...
    app.get("/api/auth/oauth/login", async (req, res) => {
        try {
            ...
            const [rows] = await db_2.pool.execute('SELECT qauth_api, qauth_appkey FROM oauth_config WHERE id = 1');
                    ...
                    if (global.__oauth_states) {
                        ...
                        for (const [key, val] of global.__oauth_states.entries()) {
                            if (now - val.createdAt > 5 * 60 * 1000)
                                global.__oauth_states.delete(key);
                        }
                    }
                }, 5 * 60 * 1000);
            }
    ...
    app.post("/api/auth/sub-login", async (req, res) => {
        ...
        captchaVerifiedTokens.delete(captchaToken);
        try {
            const subUser = await db_1.subUserDb.findByEmail(email);
            if (!subUser) {
                return res.status(401).json({ success: false, message: '邮箱或密码错误' });
            }
            if (!subUser.is_enabled) {
                console.log('🔴 子账号被禁用:', { subUserId: subUser.id, parent_user_id: subUser.parent_user_id });
                const parentUser = await db_1.userDb.findById(subUser.parent_user_id);
            ...
            const isValid = await (0, db_1.verifyPassword)(password, subUser.password_hash);
            if (!isValid) {
                return res.status(401).json({ success: false, message: '邮箱或密码错误' });
            }
            await db_1.subUserDb.updateLastLogin(subUser.id);
            // 获取主账号的积分
            const parentUser = await db_1.userDb.findById(subUser.parent_user_id);
            if (!parentUser) {
            ...
            // 创建会话，关联sub_user_id
            const connection = await db_2.pool.getConnection();
            try {
                const expiresAt = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000);
                await connection.execute('INSERT INTO sessions (user_id, sub_user_id, token, expires_at, ip_address) VALUES (?, ?, ?, ?, ?)', [subUser.parent_user_id, subUser.id, token, expiresAt, ipAddress]);
            }
            finally {
                connection.release();
            }
    ...
    // 获取子账号列表（含配额信息）
    app.get("/api/auth/sub-users", authMiddleware, async (req, res) => {
        try {
            const userId = req.user.id;
            ...
            let mainCredits = 0;
            if (isMainAccount) {
                const [userRows] = await db_2.pool.execute('SELECT credits, sub_quota_mode FROM users WHERE id = ?', [userId]);
                mainCredits = parseFloat(userRows[0]?.credits) || 0;
                quotaMode = userRows[0]?.sub_quota_mode || 'shared';
            }
            const subUsersResult = await db_1.subUserDb.getByParentUserId(req.user.id);
            const subUsers = Array.isArray(subUsersResult) ? subUsersResult : [];
            // 获取每个子账号的积分消耗（从交易记录统计的冗余数据，现在用 quota_consumed）
            const subUsersWithQuota = subUsers.map((subUser) => {
                const quotaLimit = Number(subUser.quota_limit || 0);
                const quotaConsumed = Number(subUser.quota_consumed || 0);
                return {
                    id: subUser.id,
    ...
    // 创建子账号（主账号直接创建）
    app.post("/api/auth/sub-users", authMiddleware, async (req, res) => {
        try {
            // 只有主账号可以创建子账号
            if (req.user.isSubUser) {
            ...
            // 检查邮箱是否已存在
            const existingSubUser = await db_1.subUserDb.findByEmail(email);
            if (existingSubUser) {
                return res.status(400).json({ success: false, message: '该邮箱已被注册' });
            }
            // 创建子账号
            const passwordHash = await (0, db_1.hashPassword)(password);
            await db_1.subUserDb.create(req.user.id, email, passwordHash, name);
    ...
    // 禁用/启用子账号
    app.post("/api/auth/sub-users/:id/toggle", authMiddleware, async (req, res) => {
        try {
            const subUserId = parseInt(req.params.id);
            const { isEnabled } = req.body;
            const subUser = await db_1.subUserDb.findById(subUserId);
            if (!subUser || subUser.parent_user_id !== req.user.id) {
                return res.status(403).json({ success: false, message: '无权限操作' });
            }
            await db_1.subUserDb.toggleEnabled(subUserId, isEnabled);
            res.json({ success: true, message: isEnabled ? '已启用' : '已禁用' });
        }
        catch (error) {
    ...
    // 删除子账号
    app.delete("/api/auth/sub-users/:id", authMiddleware, async (req, res) => {
        try {
            const subUserId = parseInt(req.params.id);
            const subUser = await db_1.subUserDb.findById(subUserId);
            if (!subUser || subUser.parent_user_id !== req.user.id) {
            ...
            await db_1.subUserDb.delete(subUserId);
    ...
    app.put("/api/auth/sub-users/quota-mode", authMiddleware, async (req, res) => {
        try {
            if (req.user.isSubUser) {
                return res.status(403).json({ success: false, message: '无权限操作' });
            }
            ...
            if (mode === 'allocated') {
                const [userRows] = await db_2.pool.execute('SELECT credits FROM users WHERE id = ?', [userId]);
                const credits = userRows[0]?.credits || 0;
                const totalAllocated = await db_1.subUserDb.getTotalAllocatedQuota(userId);
            ...
            await db_2.pool.execute('UPDATE users SET sub_quota_mode = ? WHERE id = ?', [mode, userId]);
            const [updated] = await db_2.pool.execute('SELECT credits, sub_quota_mode FROM users WHERE id = ?', [userId]);
    ...
    // 设置子账号配额
    app.put("/api/auth/sub-users/:id/quota", authMiddleware, async (req, res) => {
        try {
            if (req.user.isSubUser) {
                return res.status(403).json({ success: false, message: '无权限操作' });
            }
            ...
            const [userRows] = await db_2.pool.execute('SELECT credits FROM users WHERE id = ?', [userId]);
            const credits = userRows[0]?.credits || 0;
            const otherAllocated = await db_1.subUserDb.getTotalAllocatedQuota(userId) - Number(subUser.quota_limit || 0);
            ...
            await db_1.subUserDb.updateQuota(subUserId, quotaNum);
            res.json({
                success: true,
    ...
    // 追加子账号配额
    app.post("/api/auth/sub-users/:id/quota/add", authMiddleware, async (req, res) => {
        try {
            if (req.user.isSubUser) {
                return res.status(403).json({ success: false, message: '无权限操作' });
            }
            ...
            const otherAllocated = await db_1.subUserDb.getTotalAllocatedQuota(userId) - currentQuota;
            ...
            await db_2.pool.execute('UPDATE sub_users SET quota_limit = quota_limit + ? WHERE id = ?', [addAmount, subUserId]);
            const [updated] = await db_2.pool.execute('SELECT quota_limit, quota_consumed FROM sub_users WHERE id = ?', [subUserId]);
    ...
    // 获取主账号的总消费（包括所有子账号）
    app.get("/api/auth/total-consumption", authMiddleware, async (req, res) => {
        try {
            // 只有主账号可以查看
            if (req.user.isSubUser) {
            ...
            const userId = req.user.id;
            // 获取主账号的消费记录
            const [mainConsumption] = await db_2.pool.execute('SELECT SUM(ABS(amount)) as total FROM credit_transactions WHERE user_id = ? AND amount < 0', [userId]);
            // 获取所有子账号的消费记录
            const [subConsumption] = await db_2.pool.execute('SELECT SUM(ABS(amount)) as total FROM credit_transactions WHERE parent_user_id = ? AND amount < 0', [userId]);
            const mainTotal = mainConsumption[0]?.total || 0;
            const subTotal = subConsumption[0]?.total || 0;
            res.json({
                success: true,
                consumption: {
                    main: mainTotal,
    ...
    // 扣费端点
    app.post("/api/credits/deduct", authMiddleware, async (req, res) => {
        try {
            ...
            const result = await db_1.creditTransactionDb.deduct(parentUserId, amount, type || 'generate', description || '图片生成', parentUserId, subUserId);
            if (!result.success) {
            ...
            const updatedUser = await db_1.userDb.findById(parentUserId);
    ...
    // ==================== 代理系统 API ====================
    // 生成邀请码
    app.post("/api/agent/invite-code", authMiddleware, async (req, res) => {
        try {
            if (req.user.isSubUser) {
                return res.status(403).json({ success: false, message: '子账号无权限' });
            }
            ...
            const code = await db_1.inviteCodeDb.create(userId);
    ...
    // 查看邀请码列表
    app.get("/api/agent/invite-codes", authMiddleware, async (req, res) => {
        try {
            if (req.user.isSubUser) {
                return res.status(403).json({ success: false, message: '子账号无权限' });
            }
            const codes = await db_1.inviteCodeDb.getByParentUserId(req.user.id);
    ...
    // 查看佣金概览和流水
    app.get("/api/agent/commission", authMiddleware, async (req, res) => {
        try {
            if (req.user.isSubUser) {
                return res.status(403).json({ success: false, message: '子账号无权限' });
            }
            ...
            const user = await db_1.userDb.findById(userId);
            const commissionData = await db_1.commissionDb.getByAgentId(userId, page, pageSize);
            res.json({
                success: true,
    ...
    // 查看我邀请的客户
    app.get("/api/agent/customers", authMiddleware, async (req, res) => {
        try {
            if (req.user.isSubUser) {
                return res.status(403).json({ success: false, message: '子账号无权限' });
            }
            const page = parseInt(req.query.page) || 1;
            const result = await db_1.userDb.findAgentCustomers(req.user.id, page);
    ...
    // 获取代理自定义定价
    app.get("/api/agent/pricing", authMiddleware, async (req, res) => {
        try {
            if (req.user.isSubUser) {
                return res.status(403).json({ success: false, message: '子账号无权限' });
            }
            const agentId = req.query.agentId ? parseInt(req.query.agentId) : req.user.id;
            // 管理员可以查看任意代理的定价，普通代理只能看自己的
            if (agentId !== req.user.id && !req.user.isAdmin) {
                return res.status(403).json({ success: false, message: '无权限' });
            }
            const pricing = await db_1.agentDb.getPricing(agentId);
            // 同时返回定价配置的名称
            const [configRows] = await db_2.pool.execute('SELECT `key`, name, price FROM pricing_config WHERE enabled = 1');
    ...
    // 设置代理自定义定价
    app.put("/api/agent/pricing", authMiddleware, async (req, res) => {
        try {
            if (req.user.isSubUser) {
                return res.status(403).json({ success: false, message: '子账号无权限' });
            }
            ...
            // 确保是代理
            const user = await db_1.userDb.findById(req.user.id);
            if (!user?.is_agent) {
                return res.status(403).json({ success: false, message: '您不是代理，无权限设置定价' });
            }
            // 验证定价不能低于官方价格
            const [pricingRows] = await db_2.pool.execute('SELECT `key`, price FROM pricing_config WHERE enabled = 1');
            const defaultPrices = {};
            for (const row of pricingRows) {
                defaultPrices[row.key] = parseFloat(row.price);
            }
            const errors = [];
            ...
            await db_1.agentDb.setPricing(req.user.id, pricing);
    ...
    // 提交提现申请
    // 申请成为代理
    app.post("/api/agent/apply", authMiddleware, async (req, res) => {
        try {
            if (req.user.isSubUser) {
                return res.status(403).json({ success: false, message: '子账号无权限' });
            }
            ...
            await db_2.pool.execute('UPDATE users SET applied_agent = 1 WHERE id = ?', [userId]);
            // 发送邮件通知管理员
            const adminEmail = process.env.ADMIN_EMAIL;
            if (adminEmail) {
                const transporter = nodemailer_1.default.createTransport({
    ...
    app.post("/api/agent/withdraw", authMiddleware, async (req, res) => {
        try {
            if (req.user.isSubUser) {
                return res.status(403).json({ success: false, message: '子账号无权限' });
            }
            ...
            // 扣除佣金余额
            await db_2.pool.execute('UPDATE users SET commission_balance = commission_balance - ? WHERE id = ?', [amount, userId]);
            // 创建提现申请
            await db_1.withdrawDb.create(userId, amount, accountType, accountId);
            // 异步发送邮件通知管理员
    ...
    // 查看提现记录
    app.get("/api/agent/withdraw-logs", authMiddleware, async (req, res) => {
        try {
            if (req.user.isSubUser) {
                return res.status(403).json({ success: false, message: '子账号无权限' });
            }
            const logs = await db_1.withdrawDb.getByAgentId(req.user.id);
    ...
    // ==================== Banner轮播图历史 API ====================
    // 保存Banner轮播图记录
    app.post("/api/banner-carousel", authMiddleware, async (req, res) => {
        try {
            const userId = req.user.id;
            const parentUserId = req.user.parentUserId || req.user.id;
            ...
            const recordId = await db_1.bannerCarouselDb.create(userId, {
                productImages,
                productDescription,
                analysisResult,
                generatedImages,
    ...
    // 获取Banner轮播图历史记录列表
    app.get("/api/banner-carousel", authMiddleware, async (req, res) => {
        try {
            const userId = req.user.id;
            const page = parseInt(req.query.page) || 1;
            const pageSize = parseInt(req.query.pageSize) || 10;
            const result = await db_1.bannerCarouselDb.getByUserId(userId, page, pageSize);
            res.json({ success: true, ...result });
        }
        catch (error) {
    ...
    // 获取单条Banner轮播图记录详情
    app.get("/api/banner-carousel/:id", authMiddleware, async (req, res) => {
        try {
            const userId = req.user.id;
            const recordId = parseInt(req.params.id);
            const record = await db_1.bannerCarouselDb.getById(recordId, userId);
            if (!record) {
    ...
    // 删除Banner轮播图记录
    app.delete("/api/banner-carousel/:id", authMiddleware, async (req, res) => {
        try {
            const userId = req.user.id;
            const recordId = parseInt(req.params.id);
            await db_1.bannerCarouselDb.delete(recordId, userId);
    ...
    // ==================== 图片库 API ====================
    // 保存图片到数据库
    app.post("/api/images/library", authMiddleware, async (req, res) => {
        try {
            const userId = req.user.id;
            const parentUserId = req.user.parentUserId || req.user.id;
            const subUserId = req.user.isSubUser ? userId : undefined;
            ...
            if (isVideo || imageUrl.includes('.mp4') || imageUrl.includes('video')) {
                ...
                try {
                    const videoResponse = await axios_1.default.get(imageUrl, {
            ...
            let imageId = null;
            const [existing] = await db_2.pool.execute('SELECT id, type FROM generated_images WHERE user_id = ? AND image_url = ? LIMIT 1', [userId, finalUrl]);
            if (existing.length > 0) {
                imageId = existing[0].id;
                const existingType = existing[0].type;
                // 如果前端传了新type且与现有type不同，更新type
                if (type && existingType !== type) {
                    await db_2.pool.execute('UPDATE generated_images SET type = ? WHERE id = ?', [type, imageId]);
                ...
                imageId = await db_1.generatedImagesDb.create(userId, finalUrl, prompt || '', {
                    model: displayModel,
                    type: type || 'generated',
            ...
            if (skipDeduct) {
                ...
                try {
                    ...
                    const deductResult = await db_1.creditTransactionDb.deduct(parentUserId, deductAmount, deductType, deductDesc, parentUserId, subUserId);
                    if (deductResult.success) {
                        console.log('✅ 扣费成功');
                        // 代理佣金
    ...
    // 获取用户的图片列表（分页）
    app.get("/api/images/library", authMiddleware, async (req, res) => {
        try {
            const page = parseInt(req.query.page) || 1;
            const pageSize = parseInt(req.query.pageSize) || 20;
            ...
            const userId = req.user.id;
            const parentUserId = req.user.parentUserId || req.user.id;
            const result = await db_1.generatedImagesDb.getByUserId(userId, parentUserId, page, pageSize, filter);
            // 规范化模型名称并添加自动删除提示
    ...
    // 删除图片记录
    app.delete("/api/images/library/:id", authMiddleware, async (req, res) => {
        try {
            const userId = req.user.id;
            const parentUserId = req.user.parentUserId || req.user.id;
            ...
            const [rows] = await db_2.pool.execute(`SELECT image_url, thumbnail_url FROM generated_images WHERE id = ? AND ${userCondition}`, queryParams);
            ...
            await db_2.pool.execute(`DELETE FROM generated_images WHERE id = ? AND ${userCondition}`, queryParams);
    ...
    // 通过URL删除图片（前端画布删除用）
    app.post("/api/images/delete-by-url", authMiddleware, async (req, res) => {
        try {
            const userId = req.user.id;
            const parentUserId = req.user.parentUserId || req.user.id;
            const { url } = req.body;
            ...
            // 查找数据库记录
            const [rows] = await db_2.pool.execute('SELECT id, image_url FROM generated_images WHERE image_url = ? AND (user_id = ? OR parent_user_id = ?) LIMIT 1', [url, userId, parentUserId]);
            const image = rows[0];
            if (image) {
                // 从COS删除图片
                ...
                await db_2.pool.execute('DELETE FROM generated_images WHERE id = ? AND (user_id = ? OR parent_user_id = ?)', [image.id, userId, parentUserId]);
    ...
    // 批量删除图片记录
    app.post("/api/images/library/batch-delete", authMiddleware, async (req, res) => {
        try {
            const userId = req.user.id;
            const parentUserId = req.user.parentUserId || req.user.id;
            const { ids } = req.body;
            ...
            const [rows] = await db_2.pool.execute(`SELECT id, image_url FROM generated_images WHERE id IN (${placeholders}) AND ${userCondition}`, queryParams);
            const images = rows;
            // 从COS批量删除图片
            let deletedCount = 0;
            ...
            // 从数据库批量删除
            await db_2.pool.execute(`DELETE FROM generated_images WHERE id IN (${placeholders}) AND ${userCondition}`, queryParams);
    ...
    // 更新图片在画布上的位置
    app.post("/api/images/update-positions", authMiddleware, async (req, res) => {
        ...
        try {
            await db_1.generatedImagesDb.batchUpdatePositions(positions, userId);
    ...
    app.post('/api/images/library/cleanup', authMiddleware, async (req, res) => {
        try {
            const userId = req.user?.id;
            if (!userId) {
            ...
            const [expired] = await db_2.pool.execute(`SELECT id, image_url FROM generated_images WHERE ${userCondition} AND created_at < DATE_SUB(NOW(), INTERVAL 3 DAY)`, queryParams);
            const expiredImages = expired;
            let cosDeleted = 0;
            ...
            const [result] = await db_2.pool.execute(`DELETE FROM generated_images WHERE ${userCondition} AND created_at < DATE_SUB(NOW(), INTERVAL 3 DAY)`, queryParams);
    ...
    // 手动触发清理过期图片（管理员功能）
    app.post("/api/images/cleanup-expired", authMiddleware, async (req, res) => {
        try {
            const { cleanupExpiredImages } = await Promise.resolve().then(() => __importStar(require('./src/server/cleanup-expired-images')));
    ...
    app.post("/api/images/generations", authMiddleware, async (req, res) => {
        ...
        try {
            ...
            const mainUser = await db_1.userDb.findById(parentUserId);
            ...
            try {
                ...
                if (resp.data?.data) {
                    rawImageUrls = resp.data.data.map((item) => item.url).filter(Boolean);
                }
            }
            catch (e) {
                console.log('generations failed:', e.message);
            }
            if (rawImageUrls.length === 0)
                return res.status(500).json({ error: '图片生成失败，未获取到图片URL' });
            // ★★★ 同步下载图片、上传COS、写库，返回COS URL ★★★
            const actualCost = COST * rawImageUrls.length;
            const modelLabel = isGptImage2 ? '(GPT-Image2)' : '';
            let cosUrls = [];
            try {
                cosUrls = await Promise.all(rawImageUrls.map(async (rawUrl) => {
                    for (let attempt = 1; attempt <= 3; attempt++) {
                        try {
                            const ir = await axios_1.default.get(rawUrl, { responseType: 'arraybuffer', timeout: 30000,
            ...
            // 同步写库（用COS URL）
            async function saveWithRetry(url, retries = 2) {
                for (let attempt = 0; attempt <= retries; attempt++) {
                    try {
                        return await db_1.generatedImagesDb.create(userId, url, prompt, {
            ...
            const deductResult = await db_1.creditTransactionDb.deduct(parentUserId, actualCost, 'generate', `图片生成${modelLabel}${rawImageUrls.length > 1 ? `(x${rawImageUrls.length})` : ''}${req.user.isSubUser ? '(子账号)' : ''}`, parentUserId, subUserId);
            if (!deductResult.success) {
            ...
            res.status(error.response?.status || 500).json({ error: error.response?.data?.error?.message || error.response?.data?.error || error.response?.data?.message || error.message || '图片生成失败' });
        }
    });
    // API Route for Image Edit
    app.post("/api/images/edits", authMiddleware, async (req, res) => {
        ...
        if (bodyForLog.images) {
            bodyForLog.images = bodyForLog.images.map((img) => img?.startsWith('data:image/') ? `[base64 ${img.substring(11, 20)}...]` : img?.substring(0, 100));
        }
        ...
        try {
            ...
            for (let i = 0; i < images.length; i++) {
                ...
                try {
                    if (typeof imageInput === 'string' && imageInput.startsWith('data:image/')) {
                        ...
                        const imageResponse = await axios_1.default.get(imageInput, {
                            responseType: 'arraybuffer',
                            timeout: 300000,
                            headers: {
            ...
            for (let retryCount = 0; retryCount <= maxRetries; retryCount++) {
                try {
                    if (apiModel === 'gpt-image-2') {
                        ...
                        const editFormData = new form_data_1.default();
                        ...
                        const editResponse = await axios_1.default.post("https://api.xgapi.top/v1/images/edits", editFormData, {
                            headers: {
                        ...
                        const chatResponse = await axios_1.default.post("https://api.xgapi.top/v1/chat/completions", chatPayload, {
                            headers: {
                                'Authorization': `Bearer ${API_KEY}`,
            ...
            const rawUrls = imageItems.map((item) => item.url).filter(Boolean);
            if (rawUrls.length === 0) {
            ...
            let cosUrls = [];
            try {
                cosUrls = await Promise.all(rawUrls.map(async (rawUrl) => {
                    for (let attempt = 1; attempt <= 3; attempt++) {
                        try {
                            const ir = await axios_1.default.get(rawUrl, {
            ...
            // 同步写库（用COS URL）
            async function saveEditWithRetry(url, retries = 2) {
                for (let attempt = 0; attempt <= retries; attempt++) {
                    try {
                        return await db_1.generatedImagesDb.create(userId, url, prompt, {
            ...
            // 同步扣费 + 佣金（先扣费再返回结果，防止白嫖）
            const deductResult = await db_1.creditTransactionDb.deduct(parentUserId, actualEditCost, 'edit', `图片编辑${modelLabel}${req.user.isSubUser ? '(子账号)' : ''}`, parentUserId, subUserId);
            if (!deductResult.success) {
    ...
    // ==================== 产品融图 API ====================
    // 产品融合生成
    app.post("/api/product-fusion/generate", authMiddleware, async (req, res) => {
...
输出要求：画面清晰锐利，产品与背景光影一致，融合自然真实，细节丰富`;
            // 调用图片生成API
            const response = await axios_1.default.post("https://api.xgapi.top/v1/images/generations", {
                prompt: fusionPrompt,
            ...
            if (response.data && response.data.length > 0) {
                ...
                const deductResult = await db_1.creditTransactionDb.deduct(parentUserId, COST, 'product_fusion', `产品融图-${scene}${req.user.isSubUser ? '(子账号)' : ''}`, parentUserId, subUserId);
                if (!deductResult.success) {
                    return res.status(400).json({ error: '扣费失败: ' + (deductResult.message || '') });
                }
                // 代理佣金
                ...
                (async () => {
                    try {
                        ...
                        const imageResponse = await axios_1.default.get(generatedUrl, {
                            responseType: 'arraybuffer',
                            headers: {
                        ...
                        // 先保存临时URL到数据库
                        await db_1.generatedImagesDb.create(userId, generatedUrl, `产品融图-${scene}`, {
                            model: 'nano',
                        ...
                        // 更新为COS URL
                        const updatedRows = await db_1.generatedImagesDb.updateUrlByTempUrl(generatedUrl, cosUrl);
                        if (updatedRows === 0) {
    ...
    // ==================== PDF文件上传接口 ====================
    app.post("/api/upload/pdf", authMiddleware, async (req, res) => {
        try {
    ...
    app.post("/api/chat/deepseek", authMiddleware, async (req, res) => {
        try {
            ...
            const chatUserId = req.user?.isSubUser ? req.user?.parentUserId : req.user?.id;
            if (chatUserId) {
                const [users] = await db_2.pool.execute('SELECT credits FROM users WHERE id = ?', [chatUserId]);
            ...
            const response = await axios_1.default.post(apiUrl, requestData, {
                headers: {
                    'Authorization': `Bearer ${API_KEY}`,
                    'Content-Type': 'application/json',
                },
            ...
            try {
                if (chatUserId) {
                    await db_2.pool.execute('UPDATE users SET credits = credits - ? WHERE id = ? AND credits >= ?', [chatCredits, chatUserId, chatCredits]);
                    await db_2.pool.execute('INSERT INTO credit_transactions (user_id, amount, type, description, created_at) VALUES (?, ?, ?, ?, NOW())', [chatUserId, -chatCredits, 'consumption', `AI文案对话(${model})`]);
                    // 代理佣金
    ...
    app.post("/api/chat/deepseek/messages", authMiddleware, async (req, res) => {
        try {
            const userId = req.user?.id;
            if (!userId) {
            ...
            // 清空该用户的旧消息
            await db_2.pool.execute('DELETE FROM deepseek_chat_messages WHERE user_id = ?', [userId]);
            // 批量插入新消息
            if (messages.length > 0) {
                const values = messages.map((m) => [userId, m.type, m.content]);
                const placeholders = values.map(() => '(?, ?, ?)').join(', ');
                await db_2.pool.execute(`INSERT INTO deepseek_chat_messages (user_id, type, content) VALUES ${placeholders}`, values.flat());
            }
    ...
    app.get("/api/chat/deepseek/messages", authMiddleware, async (req, res) => {
        try {
            const userId = req.user?.id;
            if (!userId) {
                return res.status(401).json({ success: false, error: '未登录' });
            }
            const [rows] = await db_2.pool.execute('SELECT type, content, created_at FROM deepseek_chat_messages WHERE user_id = ? ORDER BY id ASC', [userId]);
    ...
    // 管理员直接充值用户积分
    app.post("/api/admin/recharge-user", adminMiddleware, async (req, res) => {
        try {
            const { email, credits } = req.body;
            if (!email || !credits || credits <= 0) {
                return res.status(400).json({ success: false, message: '邮箱和积分数量必填且必须大于0' });
            }
            const [users] = await db_2.pool.execute('SELECT id FROM users WHERE email = ?', [email]);
            if (!users || users.length === 0) {
                return res.status(404).json({ success: false, message: '用户不存在' });
            }
            const userId = users[0].id;
            // 更新用户积分
            await db_2.pool.execute('UPDATE users SET credits = credits + ? WHERE id = ?', [credits, userId]);
            await db_2.pool.execute('INSERT INTO credit_buckets (user_id, total_amount, remaining_amount, source, expires_at) VALUES (?, ?, ?, ?, NULL)', [userId, credits, credits, 'recharge']);
            // 记录交易
            await db_2.pool.execute('INSERT INTO payment_orders (user_id, order_id, amount, status, created_at) VALUES (?, ?, ?, ?, ?)', [userId, `admin-recharge-${Date.now()}`, credits, 'completed', new Date()]);
            res.json({
                success: true,
    ...
    app.get("/api/admin/pricing", adminMiddleware, async (req, res) => {
        try {
            const [rows] = await db_2.pool.execute('SELECT id, `key`, name, price, enabled, created_at, updated_at FROM pricing_config ORDER BY id');
            res.json({ success: true, data: rows });
        }
        catch (error) {
    ...
    app.put("/api/admin/pricing/:key", adminMiddleware, async (req, res) => {
        try {
            ...
            const [result] = await db_2.pool.execute('UPDATE pricing_config SET price = ?, enabled = ? WHERE `key` = ?', [price, enabled !== undefined ? (enabled ? 1 : 0) : 1, key]);
    ...
    app.put("/api/admin/site-config", adminMiddleware, async (req, res) => {
        try {
            ...
            if (site_title !== undefined) {
                ...
                params.push(site_title);
            }
            if (updates.length === 0) {
                return res.status(400).json({ success: false, message: '没有需要更新的字段' });
            }
            params.push(1);
            await db_2.pool.execute(`UPDATE site_config SET ${updates.join(', ')} WHERE id = ?`, params);
    ...
    app.get("/api/admin/oauth-config", adminMiddleware, async (req, res) => {
        try {
            const [rows] = await db_2.pool.execute('SELECT * FROM oauth_config WHERE id = 1');
            if (rows && rows.length > 0) {
                const config = rows[0];
                if (typeof config.platforms === 'string') {
                    try {
                        config.platforms = JSON.parse(config.platforms);
                    }
                    catch {
                        config.platforms = {};
                    }
    ...
    app.put("/api/admin/oauth-config", adminMiddleware, async (req, res) => {
        try {
            ...
            const updates = [];
            const params = [];
            if (qauth_api !== undefined) {
                updates.push('qauth_api = ?');
                params.push(qauth_api);
            }
            if (qauth_appkey !== undefined) {
                updates.push('qauth_appkey = ?');
                params.push(qauth_appkey);
            }
            if (qauth_user_secret !== undefined) {
                updates.push('qauth_user_secret = ?');
                params.push(qauth_user_secret);
            }
            if (qauth_auto_register !== undefined) {
                updates.push('qauth_auto_register = ?');
                params.push(qauth_auto_register ? 1 : 0);
            }
            if (qauth_state_check !== undefined) {
                updates.push('qauth_state_check = ?');
                params.push(qauth_state_check ? 1 : 0);
            }
            ...
            params.push(1);
            await db_2.pool.execute(`UPDATE oauth_config SET ${updates.join(', ')} WHERE id = ?`, params);
    ...
    app.post("/api/admin/coupons", adminMiddleware, async (req, res) => {
        try {
            ...
            await db_2.pool.execute('INSERT INTO coupons (code, credits, max_claims, claim_deadline, expire_days) VALUES (?, ?, ?, ?, ?)', [code.toUpperCase(), credits, max_claims || 0, claim_deadline, expire_days || 30]);
    ...
    app.get("/api/admin/coupons", adminMiddleware, async (req, res) => {
        try {
            ...
            const pageSize = 20;
            const offset = (page - 1) * pageSize;
            const [totalResult] = await db_2.pool.execute('SELECT COUNT(*) as total FROM coupons');
            const total = totalResult[0].total;
            const [rows] = await db_2.pool.execute('SELECT * FROM coupons ORDER BY created_at DESC LIMIT ? OFFSET ?', [pageSize, offset]);
            res.json({
                success: true,
                data: rows,
                pagination: { page, pageSize, total, totalPages: Math.ceil(total / pageSize) }
    ...
    app.put("/api/admin/coupons/:id", adminMiddleware, async (req, res) => {
        try {
            ...
            await db_2.pool.execute('UPDATE coupons SET code=?, credits=?, max_claims=?, claim_deadline=?, expire_days=?, is_active=? WHERE id=?', [code, credits, max_claims, claim_deadline, expire_days, is_active ? 1 : 0, id]);
    ...
    app.delete("/api/admin/coupons/:id", adminMiddleware, async (req, res) => {
        try {
            await db_2.pool.execute('DELETE FROM coupons WHERE id = ?', [req.params.id]);
            res.json({ success: true, message: '已删除' });
        }
        catch (error) {
    ...
    app.get("/api/admin/agents", adminMiddleware, async (req, res) => {
        try {
            const page = parseInt(req.query.page) || 1;
            const result = await db_1.userDb.getAllAgents(page);
            res.json({ success: true, ...result });
        }
        catch (error) {
    ...
    // 待审核代理申请列表
    app.get("/api/admin/agent-applications", adminMiddleware, async (req, res) => {
        try {
            const [rows] = await db_2.pool.execute("SELECT id, email, credits, created_at FROM users WHERE applied_agent = 1 AND is_agent = 0 AND is_enabled = 1 ORDER BY created_at DESC");
            res.json({ success: true, data: rows });
        }
        catch (error) {
    ...
    // 开启/关闭代理资格
    app.post("/api/admin/agents/:id/toggle", adminMiddleware, async (req, res) => {
        try {
            const id = parseInt(req.params.id);
            const { isAgent } = req.body;
            // 检查用户是否是管理员，管理员不能设为代理
            if (isAgent) {
                const user = await db_1.userDb.findById(id);
                if (!user)
                    return res.status(404).json({ success: false, message: '用户不存在' });
                if (user.is_admin) {
                ...
                // 清除申请标记，设为代理
                await db_2.pool.execute('UPDATE users SET is_agent = 1, applied_agent = 0 WHERE id = ?', [id]);
                // 发送通知邮件给用户
            ...
            `
                ...
                await db_2.pool.execute('UPDATE users SET is_agent = 0 WHERE id = ?', [id]);
    ...
    app.get("/api/admin/withdraws", adminMiddleware, async (req, res) => {
        try {
            const page = parseInt(req.query.page) || 1;
            const result = await db_1.withdrawDb.getPending(page);
            res.json({ success: true, ...result });
        }
        catch (error) {
    ...
    // 审核提现
    app.post("/api/admin/withdraws/:id/process", adminMiddleware, async (req, res) => {
        try {
            const id = parseInt(req.params.id);
            ...
            // 如果拒绝，需要退款
            if (status === 'rejected') {
                const [requests] = await db_2.pool.execute('SELECT * FROM withdraw_requests WHERE id = ?', [id]);
                const reqRecord = requests[0];
                if (reqRecord) {
                    await db_2.pool.execute('UPDATE users SET commission_balance = commission_balance + ? WHERE id = ?', [reqRecord.amount, reqRecord.agent_id]);
                }
            }
            await db_1.withdrawDb.updateStatus(id, status, remark);
    ...
    // 上传转账凭证
    app.post("/api/admin/withdraws/:id/upload-proof", adminMiddleware, async (req, res) => {
        try {
            const id = parseInt(req.params.id);
            ...
            // 更新数据库
            await db_1.withdrawDb.updateProof(id, url);
    ...
    // 删除转账凭证
    app.post("/api/admin/withdraws/:id/delete-proof", adminMiddleware, async (req, res) => {
        try {
            const id = parseInt(req.params.id);
            await db_2.pool.execute('UPDATE withdraw_requests SET proof_image_url = NULL WHERE id = ?', [id]);
    ...
    // ==================== 模特库管理 API ====================
    // 获取模特列表
    app.get("/api/admin/model-library", adminMiddleware, async (req, res) => {
        try {
            const gender = req.query.gender;
            if (!gender || !['female', 'male'].includes(gender)) {
                return res.status(400).json({ success: false, message: '请指定性别参数 (female/male)' });
            }
            const [rows] = await db_2.pool.execute('SELECT id, name, gender, image_url, created_at FROM model_library WHERE gender = ? ORDER BY created_at DESC', [gender]);
            res.json({ success: true, data: rows });
        }
        catch (error) {
            res.status(500).json({ success: false, message: error.message });
    ...
    // 上传模特图片
    app.post("/api/admin/model-library/upload", adminMiddleware, async (req, res) => {
        try {
            ...
            const [result] = await db_2.pool.execute('INSERT INTO model_library (name, gender, image_url, cos_key) VALUES (?, ?, ?, ?)', [name, gender, imageUrl, key]);
            res.json({
                success: true,
                id: result.insertId,
                name,
    ...
    // 删除模特
    app.delete("/api/admin/model-library/:id", adminMiddleware, async (req, res) => {
        try {
            const id = parseInt(req.params.id);
            const [rows] = await db_2.pool.execute('SELECT * FROM model_library WHERE id = ?', [id]);
            const model = rows[0];
            if (!model) {
            ...
            // 从数据库删除
            await db_2.pool.execute('DELETE FROM model_library WHERE id = ?', [id]);
    ...
    // ==================== 用户领券 API ====================
    // 领取优惠券
    app.post("/api/coupons/claim", authMiddleware, async (req, res) => {
        try {
            const { code } = req.body;
            const userId = req.user.id;
            if (!code) {
            ...
            // 查找优惠券
            const [couponRows] = await db_2.pool.execute('SELECT * FROM coupons WHERE code = ?', [code.toUpperCase()]);
            ...
            try {
                ...
                // 行锁校验名额
                const [lockedRows] = await connection.execute('SELECT claimed_count, max_claims FROM coupons WHERE id = ? FOR UPDATE', [coupon.id]);
                ...
                // 校验重复领取
                const [existingClaims] = await connection.execute('SELECT id FROM coupon_claims WHERE coupon_id = ? AND user_id = ?', [coupon.id, userId]);
                ...
                const [claimResult] = await connection.execute('INSERT INTO coupon_claims (coupon_id, user_id, credits, expires_at) VALUES (?, ?, ?, ?)', [coupon.id, userId, coupon.credits, expiresAt]);
                await connection.execute('UPDATE users SET credits = credits + ? WHERE id = ?', [coupon.credits, userId]);
                await connection.execute('UPDATE coupons SET claimed_count = claimed_count + 1 WHERE id = ?', [coupon.id]);
                await connection.execute('INSERT INTO credit_buckets (user_id, total_amount, remaining_amount, source, coupon_claim_id, expires_at) VALUES (?, ?, ?, ?, ?, ?)', [userId, coupon.credits, coupon.credits, 'coupon', claimResult.insertId, expiresAt]);
                await connection.commit();
                // 记录交易
                await db_2.pool.execute('INSERT INTO credit_transactions (user_id, amount, type, description) VALUES (?, ?, ?, ?)', [userId, coupon.credits, 'coupon', `优惠券领取: ${coupon.code}`]);
                res.json({
                    success: true,
    ...
    // 用户的领取记录
    app.get("/api/coupons/claims", authMiddleware, async (req, res) => {
        try {
            const [rows] = await db_2.pool.execute(`SELECT cc.*, c.code, c.expire_days
         FROM coupon_claims cc
         JOIN coupons c ON cc.coupon_id = c.id
         WHERE cc.user_id = ?
         ORDER BY cc.claimed_at DESC`, [req.user.id]);
            res.json({ success: true, data: rows });
        }
        catch (error) {
    ...
    // 画布状态保存/读取
    app.post("/api/canvas/state", authMiddleware, async (req, res) => {
        try {
            const userId = req.user.id;
            const { stateData } = req.body;
            const stateJson = JSON.stringify(stateData);
            try {
                await db_2.pool.execute(`CREATE TABLE IF NOT EXISTS canvas_states (
          id INT PRIMARY KEY AUTO_INCREMENT,
          user_id INT NOT NULL UNIQUE,
          state_data JSON,
          created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
          ...
          FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
            ...
            const [existing] = await db_2.pool.execute('SELECT id FROM canvas_states WHERE user_id = ?', [userId]);
            if (Array.isArray(existing) && existing.length > 0) {
                await db_2.pool.execute('UPDATE canvas_states SET state_data = ?, updated_at = NOW() WHERE user_id = ?', [stateJson, userId]);
            }
            else {
                await db_2.pool.execute('INSERT INTO canvas_states (user_id, state_data, created_at, updated_at) VALUES (?, ?, NOW(), NOW())', [userId, stateJson]);
            }
            res.json({ success: true });
        }
        catch (error) {
    ...
    app.get("/api/canvas/state", authMiddleware, async (req, res) => {
        try {
            const userId = req.user.id;
            try {
                await db_2.pool.execute(`CREATE TABLE IF NOT EXISTS canvas_states (
          ...
          FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
            ...
            const [rows] = await db_2.pool.execute('SELECT state_data FROM canvas_states WHERE user_id = ?', [userId]);
    ...
    app.post("/api/canvas/plugin-state", authMiddleware, async (req, res) => {
        try {
            const userId = req.user.id;
            const { pluginId, stateData } = req.body;
            const stateJson = JSON.stringify(stateData);
            try {
                await db_2.pool.execute(`CREATE TABLE IF NOT EXISTS canvas_plugin_states (
          id INT PRIMARY KEY AUTO_INCREMENT,
          user_id INT NOT NULL,
          ...
          FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
            ...
            const [existing] = await db_2.pool.execute('SELECT id FROM canvas_plugin_states WHERE user_id = ? AND plugin_id = ?', [userId, pluginId]);
            if (Array.isArray(existing) && existing.length > 0) {
                await db_2.pool.execute('UPDATE canvas_plugin_states SET state_data = ?, updated_at = NOW() WHERE user_id = ? AND plugin_id = ?', [stateJson, userId, pluginId]);
            }
            else {
                await db_2.pool.execute('INSERT INTO canvas_plugin_states (user_id, plugin_id, state_data, created_at, updated_at) VALUES (?, ?, ?, NOW(), NOW())', [userId, pluginId, stateJson]);
            }
            res.json({ success: true });
        }
        catch (error) {
    ...
    app.get("/api/canvas/plugin-state", authMiddleware, async (req, res) => {
        try {
            const userId = req.user.id;
          ...
          FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
            ...
            const [rows] = await db_2.pool.execute('SELECT state_data FROM canvas_plugin_states WHERE user_id = ? AND plugin_id = ?', [userId, pluginId]);
            if (Array.isArray(rows) && rows.length > 0) {
                const row = rows[0];
                const stateData = typeof row.state_data === 'string' ? JSON.parse(row.state_data) : row.state_data;
                res.json({ success: true, data: stateData });
    ...
    app.get("/api/tauri/update", async (req, res) => {
        try {
            ...
            const platform = req.query.platform; // darwin-aarch64, darwin-x86_64, windows-x86_64
            const arch = req.query.arch;
            const host = req.headers.host || '43.161.228.92:3001';
            const protocol = 'http';
            const baseUrl = `${protocol}://${host}`;
            // 读取更新清单文件
            const updateDir = path_1.default.join(process.cwd(), 'deploy', 'updates');
            const manifestPath = path_1.default.join(updateDir, 'update-manifest.json');
    ...
    app.get("/updates/:filename", async (req, res) => {
        try {
            ...
            const filePath = path_1.default.join(process.cwd(), 'deploy', 'updates', filename);
    ...
    app.get("/api/chat/images", authMiddleware, async (req, res) => {
        try {
            const userId = req.user.id;
            console.log('[chat/images] Loading images for user:', userId);
            const [rows] = await db_2.pool.execute(`SELECT image_url, prompt, model, aspect_ratio, created_at, type FROM generated_images
         ...
         ORDER BY created_at DESC LIMIT 200`, [userId]);
            console.log('[chat/images] Found', rows.length, 'images');
            if (rows.length > 0) {
                console.log('[chat/images] First image:', JSON.stringify(rows[0]));
            }
            res.json({ success: true, data: rows });
        }
        catch (err) {
    ...
    // 获取用户的视频任务列表
    app.get("/api/video/tasks", authMiddleware, async (req, res) => {
        try {
            const userId = req.user.id;
            const [rows] = await db_2.pool.execute(`SELECT * FROM generated_images WHERE user_id = ? AND type = 'video' ORDER BY created_at DESC LIMIT 50`, [userId]);
            const apiKey = process.env.VIDEO_GEN_API_KEY || process.env.IMAGE_GEN_API_KEY_1;
            // 修复已存储的 URL
            const fixedRows = await Promise.all(rows.map(async (row) => {
                if (!row.image_url)
                    return row;
                ...
                if (row.model === 'omni-fast' && url.includes('/v1/videos/') && url.includes('/content')) {
                    ...
                    if (extId) {
                        try {
                            const genRes = await axios_1.default.get(`https://api.xgapi.top/v1/video/generations/${extId}`, {
                                headers: { 'Authorization': `Bearer ${apiKey}` },
                                timeout: 10000
                            });
                            const genData = genRes.data;
                            const genUrl = genData?.data?.data?.[0]?.url || genData?.video_url || genData?.data?.video_url || genData?.output?.url || genData?.data?.output?.url || genData?.data?.result_url || genData?.result_url;
                            if (genUrl && !genUrl.includes('/content')) {
                                ...
                                try {
                                    ...
                                    await cosClient.send(new client_s3_1.PutObjectCommand({
                                        Bucket: process.env.COS_BUCKET,
                                        Key: fileName,
                                        Body: videoBuffer,
                                        ContentType: 'video/mp4',
                                    ...
                                    await db_2.pool.execute(`UPDATE generated_images SET image_url = ? WHERE id = ?`, [url, row.id]);
    ...
    // 查询单个视频任务状态（只读数据库）
    app.get("/api/video/query/:taskId", authMiddleware, async (req, res) => {
        try {
            const userId = req.user.id;
            const taskId = req.params.taskId;
            let [rows] = await db_2.pool.execute(`SELECT id, status, image_url, thumbnail_url, prompt, model, created_at FROM generated_images WHERE id = ? AND user_id = ? AND type = 'video'`, [taskId, userId]);
            if (rows.length === 0) {
                [rows] = await db_2.pool.execute(`SELECT id, status, image_url, thumbnail_url, prompt, model, created_at FROM generated_images WHERE user_id = ? AND type = 'video' AND prompt LIKE ? ORDER BY created_at DESC LIMIT 1`, [userId, `%[ext:${taskId}]%`]);
            }
            const task = rows[0];
            if (!task) {
                ...
                if (task.model === 'omni-fast' && storedUrl.includes('/v1/videos/') && storedUrl.includes('/content')) {
                    try {
                        ...
                        if (vidMatch && externalTaskId) {
                            const genRes = await axios_1.default.get(`https://api.xgapi.top/v1/video/generations/${externalTaskId}`, {
                                headers: { 'Authorization': `Bearer ${apiKey}` },
                                timeout: 10000
                            });
                            const genData = genRes.data;
                            ...
                            if (genUrl && !genUrl.includes('/content')) {
                                ...
                                try {
                                    ...
                                    await db_2.pool.execute(`UPDATE generated_images SET image_url = ? WHERE id = ?`, [storedUrl, task.id]);
            ...
            if (!externalTaskId) {
                ...
                if (Date.now() - createdAt > 3 * 60 * 1000) {
                    await db_2.pool.execute(`UPDATE generated_images SET status = 'failed' WHERE id = ?`, [task.id]);
                    return res.json({ success: true, data: { status: 'failed' } });
                }
            ...
            // 实时查外部API
            try {
                const statusRes = await axios_1.default.get(`https://api.xgapi.top/v1/videos/${externalTaskId}`, {
                ...
                if (done && videoUrl) {
                    ...
                    try {
                        const videoResp = await axios_1.default.get(videoUrl, { responseType: 'arraybuffer', timeout: 300000 });
                        const videoBuffer = Buffer.from(videoResp.data);
                        const now = new Date();
                        const userPath = task.prompt?.match(/parentUserId:(\d+)/)?.[1] || userId;
                        const fileName = `${userPath}/${now.getFullYear()}/${(now.getMonth() + 1).toString().padStart(2, '0')}/video-${Date.now()}-${Math.random().toString(36).substring(2, 8)}.mp4`;
                        await cosClient.send(new client_s3_1.PutObjectCommand({
                            Bucket: process.env.COS_BUCKET,
                            Key: fileName,
                    ...
                    await db_2.pool.execute(`UPDATE generated_images SET image_url = ?, status = 'completed' WHERE id = ?`, [finalUrl, task.id]);
                    // 异步提取视频封面
                    extractVideoThumbnail(finalUrl, task.id.toString()).then(thumbUrl => {
                        if (thumbUrl)
                            db_2.pool.execute(`UPDATE generated_images SET thumbnail_url = ? WHERE id = ?`, [thumbUrl, task.id]);
                    }).catch(() => { });
                    return res.json({ success: true, data: { status: 'completed', video_url: finalUrl, progress: 100 } });
                }
                if (done && !videoUrl) {
                    await db_2.pool.execute(`UPDATE generated_images SET status = 'completed' WHERE id = ?`, [task.id]);
    ...
    app.get("/api/video/media-library", authMiddleware, async (req, res) => {
        try {
            const userId = req.user.id;
            const parentUserId = req.user.parentUserId || req.user.id;
            const page = parseInt(req.query.page) || 1;
            const pageSize = parseInt(req.query.pageSize) || 20;
            const mediaType = req.query.type; // 'video', 'video-script', 'video-social', 'all'
            const result = await db_1.generatedImagesDb.getVideoMedia(userId, parentUserId, page, pageSize, mediaType);
            // 添加自动删除提示
    ...
    // 删除单条视频媒体（含COS文件删除）
    app.delete("/api/video/media/:id", authMiddleware, async (req, res) => {
        try {
            const userId = req.user.id;
            const parentUserId = req.user.parentUserId || req.user.id;
            const mediaId = parseInt(req.params.id);
            const result = await db_1.generatedImagesDb.deleteVideoMedia(mediaId, userId, parentUserId);
            if (!result) {
                return res.status(404).json({ success: false, message: '记录不存在或无权删除' });
            }
    ...
    // 批量删除视频媒体
    app.post("/api/video/media/batch-delete", authMiddleware, async (req, res) => {
        try {
            const userId = req.user.id;
            const parentUserId = req.user.parentUserId || req.user.id;
            const { ids } = req.body;
            ...
            const deletedRecords = await db_1.generatedImagesDb.batchDeleteVideoMedia(ids, userId, parentUserId);
            // 批量删除COS文件（视频+封面图）
    ...
    // 清理过期视频媒体
    app.post("/api/video/media/cleanup", authMiddleware, async (req, res) => {
        try {
            const userId = req.user.id;
            const parentUserId = req.user.parentUserId || req.user.id;
            const expiredRecords = await db_1.generatedImagesDb.cleanupExpiredVideoMedia(userId, parentUserId);
            // 删除COS文件（视频+封面图）
            let cosDeletedCount = 0;
    ...
    // 提取视频第一帧作为封面
    async function extractVideoThumbnail(videoUrl, taskId) {
        try {
            ...
            // 下载视频
            const response = await axios_1.default.get(videoUrl, { responseType: 'arraybuffer', timeout: 60000 });
            fs_1.default.writeFileSync(tmpVideo, Buffer.from(response.data));
            // 提取第一帧
            execSync(`ffmpeg -y -i "${tmpVideo}" -ss 00:00:01 -vframes 1 -q:v 2 "${tmpThumb}"`, { timeout: 15000 });
            // 上传到COS
            const thumbBuffer = fs_1.default.readFileSync(tmpThumb);
            ...
            // 清理临时文件
            try {
                fs_1.default.unlinkSync(tmpVideo);
            }
            catch { }
            try {
                fs_1.default.unlinkSync(tmpThumb);
            }
            catch { }
    ...
    app.post("/api/video/generate", authMiddleware, async (req, res) => {
        try {
            const userId = req.user.id;
            const parentUserId = req.user.parentUserId || req.user.id;
            const { model, prompt, imageUrl, aspectRatio, duration, firstFrame, lastFrame } = req.body;
            // 验证模型
            ...
            const [existingRows] = await db_2.pool.execute(`SELECT id, image_url, status FROM generated_images 
         WHERE user_id = ? AND model = ? AND prompt = ? AND type = 'video' 
         AND created_at > DATE_SUB(NOW(), INTERVAL 5 MINUTE)
         ORDER BY created_at DESC LIMIT 1`, [userId, model, prompt]);
            ...
            const [pricingRows] = await db_2.pool.execute('SELECT price FROM pricing_config WHERE `key` = ? AND enabled = 1', [pricingKey]);
            const unitPrice = pricingRows[0]?.price || 0;
            if (unitPrice > 0) {
                const [creditsRows] = await db_2.pool.execute('SELECT id, remaining_amount FROM credit_buckets WHERE user_id = ? AND (expires_at IS NULL OR expires_at > NOW()) ORDER BY expires_at ASC', [parentUserId]);
                const buckets = creditsRows;
                const totalCredits = buckets.reduce((sum, b) => sum + Number(b.remaining_amount), 0);
                ...
                for (const bucket of buckets) {
                    ...
                    await db_2.pool.execute('UPDATE credit_buckets SET remaining_amount = remaining_amount - ? WHERE id = ?', [deduct, bucket.id]);
                    remaining -= deduct;
                }
                // 记录消费
                await db_2.pool.execute('INSERT INTO credit_transactions (user_id, type, amount, description) VALUES (?, ?, ?, ?)', [parentUserId, 'video_generation', -unitPrice, `视频生成: ${model}`]);
            }
            // 先插入一条 pending 记录，返回 task_id
            const [insertResult] = await db_2.pool.execute(`INSERT INTO generated_images (user_id, parent_user_id, image_url, prompt, model, type, aspect_ratio, status, created_at, expires_at) 
         VALUES (?, ?, '', ?, ?, 'video', ?, 'pending', NOW(), DATE_ADD(NOW(), INTERVAL 3 DAY))`, [userId, parentUserId, `${prompt} [${videoDuration}s]`, model, aspectRatio || '16:9']);
            const dbTaskId = insertResult.insertId;
            console.log('视频生成参数:', { model, prompt: prompt.substring(0, 50), aspectRatio, duration: videoDuration, imageUrl: imageUrl ? '有' : '无' });
            // 通用函数：下载视频并上传到COS
            const uploadVideoToCos = async (videoUrl) => {
                try {
            ...
            if (imageUrl && imageUrl.startsWith('data:')) {
                ...
                try {
                    ...
                    const processed = await (0, sharp_1.default)(imgBuf)
                        .resize(targetW, targetH, { fit: 'contain', background: { r: 0, g: 0, b: 0, alpha: 1 } })
                        .toBuffer();
                    imgBuf = Buffer.from(processed);
                    console.log(`📸 参考图已调整为 ${targetW}x${targetH} (${aspectRatio})，保留完整内容`);
                }
                catch (e) {
                    console.error('参考图调整失败，使用原图:', e.message);
                }
                formData.append('input_reference', imgBuf, { filename: 'reference.png', contentType: 'image/png' });
            }
            axios_1.default.post('https://api.xgapi.top/v1/videos', formData, {
                headers: {
                    'Authorization': `Bearer ${apiKey}`,
                    ...formData.getHeaders()
                },
                ...
                if (!externalTaskId) {
                    ...
                    await db_2.pool.execute(`UPDATE generated_images SET status = 'failed' WHERE id = ?`, [dbTaskId]);
                    return;
                }
                console.log('视频任务已创建，外部ID:', externalTaskId);
                await db_2.pool.execute(`UPDATE generated_images SET prompt = CONCAT(prompt, ' [ext:', ?, ']') WHERE id = ?`, [externalTaskId, dbTaskId]);
                const pollVideo = async (attempts = 0) => {
                    if (attempts >= 60) {
                    ...
                    try {
                        ...
                        if (videoUrl || status === 'SUCCESS' || status === 'completed' || status === 'succeeded') {
                            if (videoUrl) {
                                ...
                                await db_2.pool.execute(`UPDATE generated_images SET image_url = ?, status = 'completed' WHERE id = ?`, [finalUrl, dbTaskId]);
                                console.log('✅ 视频生成完成:', finalUrl);
                            }
                            else {
                                await db_2.pool.execute(`UPDATE generated_images SET status = 'completed' WHERE id = ?`, [dbTaskId]);
                            ...
                            setTimeout(() => pollVideo(attempts + 1), 5000);
                        }
                    }
                    catch (pollErr) {
    ...
    app.get("/api/workflows", authMiddleware, async (req, res) => {
        try {
            const userId = req.user.isSubUser ? req.user.parentUserId : req.user.id;
            const workflows = await db_1.workflowDb.getByUserId(userId);
    ...
    app.post("/api/workflows", authMiddleware, async (req, res) => {
        try {
            ...
            const id = await db_1.workflowDb.create(userId, name || '未命名工作流', JSON.stringify(nodes || '[]'), JSON.stringify(connections || '[]'));
    ...
    app.put("/api/workflows/:id", authMiddleware, async (req, res) => {
        try {
            ...
            const { id } = req.params;
            const { name, nodes, connections } = req.body;
            await db_1.workflowDb.update(parseInt(id), userId, name || '未命名工作流', JSON.stringify(nodes || '[]'), JSON.stringify(connections || '[]'));
            res.json({ success: true });
        }
        catch (error) {
    ...
    app.delete("/api/workflows/:id", authMiddleware, async (req, res) => {
        try {
            const userId = req.user.isSubUser ? req.user.parentUserId : req.user.id;
            const { id } = req.params;
            await db_1.workflowDb.delete(parseInt(id), userId);
            res.json({ success: true });
        }
        catch (error) {
    ...
    const hasFrontendSources = fs_1.default.existsSync("src/main.tsx");
    if (process.env.NODE_ENV !== "production" && hasFrontendSources) {
        ...
        app.use("/updates", express_1.default.static(updatesDir));
    ...
    app.listen(PORT, "0.0.0.0", () => {
        ...
        // 异步启动后台任务（不阻塞服务器启动）
        setTimeout(async () => {
            (0, cleanup_expired_images_1.startCleanupScheduler)();
            try {
                await db_2.pool.execute("ALTER TABLE users ADD COLUMN recharge_disabled TINYINT(1) DEFAULT 0 AFTER is_enabled");
            ...
            const cleanupOldContent = async () => {
                try {
                    const [rows] = await db_2.pool.execute(`SELECT id, image_url, thumbnail_url FROM generated_images WHERE created_at < DATE_SUB(NOW(), INTERVAL 5 DAY)`);
                    const items = rows;
                    if (items.length === 0)
                        return;
                    ...
                    for (const item of items) {
                        ...
                        if (item.image_url && item.image_url.includes(process.env.COS_PUBLIC_URL || '')) {
                            try {
                                ...
                                await cosClient.send(new client_s3_1.DeleteObjectCommand({ Bucket: process.env.COS_BUCKET, Key: key }));
                            }
                            catch { }
                        }
                        // 删除COS封面
                        ...
                        // 删除数据库记录
                        await db_2.pool.execute(`DELETE FROM generated_images WHERE id = ?`, [item.id]);
                    }
