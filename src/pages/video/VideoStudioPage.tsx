...
export default function VideoStudioPage() {
  ...
  const [videoInitialImages, setVideoInitialImages] = useState<string[] | undefined>(undefined);

  const handleSendToVideo = useCallback((prompt: string, images: string[]) => {
    setVideoInitialPrompt(prompt);
    setVideoInitialImages(images);
    setActiveTab('video');
  }, []);
  ...
  const renderTab = () => {
    switch (activeTab) {
      case 'script':
        return <ScriptTab onSendToVideo={handleSendToVideo} />;
      case 'social':
        return <SocialTab />;
      case 'video':
        return <VideoTab onTaskCountChange={handleTaskCountChange} onTasksChange={handleTasksChange} onCreditsChange={refreshUser} initialPrompt={videoInitialPrompt} initialImages={videoInitialImages} />;
    }
  };
