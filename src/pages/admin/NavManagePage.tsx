...
export default function NavManagePage({ token }: { token: string }) {
    ...
    try {
      ...
      if (data.success) {
        ...
        const ALL_NAVS = [
          ...
          { nav_id: 'carousel', label: '独立站轮播图', category: '店铺上架素材', sort_order: 1 },
          { nav_id: 'detail2', label: '详情页设计', category: '店铺上架素材', sort_order: 2 },
          { nav_id: 'banner', label: 'Banner设计', category: '店铺上架素材', sort_order: 3 },
          { nav_id: 'poster', label: '智能海报设计', category: '店铺上架素材', sort_order: 4 },
  ...
  const CAT_ORDER = ['素材工作台', '店铺上架素材', '社媒图文引流', '短视频带货引流', 'AI辅助工具'];
  const categories = [...new Set(items.map(i => i.category))].sort((a, b) => CAT_ORDER.indexOf(a) - CAT_ORDER.indexOf(b))
