...
export const MobileRecharge: React.FC<MobileRechargeProps> = ({ onBack }) => {
    ...
    if (params.get('sync') === '1') {
      const syncCredits = async () => {
        try {
          ...
          if (!token) return;
          await fetch(`${API_URL}/api/payment/sync-credits`, { method: 'POST', headers: { Authorization: `Bearer ${token}` } });
          refreshUser();
          const r = await fetch(`${API_URL}/api/auth/credits`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleRecharge = async () => {
    ...
    try {
      ...
      const r = await fetch(`${API_URL}/api/payment/initiate`, {
        method: 'POST',
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="px-4 pt-4 pb-8 space-y-5">
          ...
          <div className="bg-gray-50 rounded-2xl border border-gray-200 p-4">
            <p className="text-xs font-semibold text-gray-500 mb-3">消耗单价参考</p>
              ...
              {[
                ...
                <div key={item.label} className={`${item.bg} rounded-xl px-3 py-2.5 flex items-center gap-2.5`}>
                  ...
                  <div>
                    ...
                    <p className="text-xs font-semibold text-gray-700">{item.price}积分/次</p>
                  </div>
                </div>
              ))}
            </div>
              ...
              {PLANS.map(plan => {
                ...
                return (
                  <button key={plan.id} onClick={() => { setSelectedPlan(plan.id); setCustomAmount(''); }}
                    className={`w-full relative bg-gray-50 rounded-2xl p-4 border-2 transition-all text-left ${
                      sel ? 'border-blue-500 shadow-lg shadow-blue-500/25' : 'border-gray-200'
                    }`}
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-xs text-gray-400 mt-0.5">¥{plan.amount} / {plan.credits}积分</p>
                      </div>
                      <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${sel ? 'bg-blue-500 border-blue-500' : 'border-gray-300'}`}>
                        {sel && <Check size={14} className="text-white" />}
                      </div>
                    </div>
                    <div className="mt-2 bg-gray-100 rounded-xl px-3 py-2 flex items-center justify-between">
                      <span className="text-xs text-gray-500">约 <span className="font-semibold text-[#171717]">{genCount}</span> 次Nano生图</span>
                      <span className="text-xs text-gray-400">{plan.credits}积分</span>
                    </div>
                  </button>
                );
              })}
          ...
          {/* 自定义金额 */}
          <div className="bg-gray-50 rounded-2xl border border-gray-200 p-4">
            <label className="text-xs font-semibold text-gray-500 mb-2 block">自定义金额</label>
            <div className="flex items-center gap-2">
              ...
              <input type="number" value={customAmount} onChange={e => setCustomAmount(e.target.value)} placeholder="输入金额"
                className="flex-1 px-3 py-2.5 bg-gray-100 rounded-xl text-sm text-[#171717] placeholder-gray-400 outline-none" min="1" step="0.1" />
            </div>
            {customAmount && <p className="text-xs text-gray-400 mt-1.5">可获得约 <span className="font-semibold text-[#171717]">{Math.floor(Number(customAmount) / (nanoPrice || 0.3))}</span> 次Nano生图</p>}
          </div>

          {/* 支付方式 */}
          <div>
            <label className="text-xs font-semibold text-gray-500 mb-2 block">支付方式</label>
            <div className="flex gap-2">
              <button onClick={() => setPaymentMethod('wechat')}
                className={`mobile-tap flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl border-2 text-sm font-medium transition-all ${
                  paymentMethod === 'wechat' ? 'border-blue-500 bg-blue-500/10' : 'border-gray-200 bg-gray-50'
                }`}>
              ...
              <button onClick={() => setPaymentMethod('alipay')}
                className={`mobile-tap flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl border-2 text-sm font-medium transition-all ${
                  paymentMethod === 'alipay' ? 'border-blue-500 bg-blue-500/10' : 'border-gray-200 bg-gray-50'
                }`}>
          ...
          <div className="flex items-center justify-center gap-4 pt-2">
            <button className="text-[11px] text-gray-400 flex items-center gap-1"><Shield size={11} /> 隐私</button>
            <button className="text-[11px] text-gray-400 flex items-center gap-1"><FileText size={11} /> 条款</button>
            <a href="mailto:softhooky@163.com" className="text-[11px] text-gray-400 flex items-center gap-1"><Mail size={11} /> 联系</a>
          </div>
        </div>
      </div>
    </div>
