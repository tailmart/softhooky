    ...
    <div className="flex flex-col h-full bg-[#FAFAFA]">
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="px-4 pt-4 pb-32 space-y-4">
          {config.description && <p className="text-xs text-[#a3a3a3]">{config.description}</p>}

          {/* Upload */}
          <div>
            <label className="text-xs font-semibold text-[#999] mb-2 block">上传参考图 <span className="text-[#bdbdbd]">（可选）</span></label>
            <div className="flex gap-2.5 flex-wrap">
              ...
              {images.length < 3 && <button onClick={() => fileRef.current?.click()} className="w-[72px] h-[72px] rounded-xl border-2 border-dashed border-[#ddd] flex flex-col items-center justify-center bg-white/50"><Plus size={20} className="text-[#bbb]" /><span className="text-[9px] text-[#bbb]">上传</span></button>}
            </div>
          ...
          {config.models && config.models.length > 0 && (
            <div>
              ...
              <button onClick={() => setSheet('model')} className="w-full flex items-center justify-between px-4 py-3 bg-white rounded-xl border border-[#eee] text-sm">
                ...
                <div className="fixed inset-0 z-[100] flex items-end" onClick={() => setSheet(null)}>
                  ...
                  <div className="relative w-full bg-white rounded-t-3xl pb-[calc(16px+env(safe-area-inset-bottom))] animate-mobile-slide-up" onClick={e => e.stopPropagation()}>
                    <div className="flex items-center justify-between px-5 pt-4 pb-3 border-b border-[#f0f0f0]">
                      ...
                      <button onClick={() => setSheet(null)} className="w-8 h-8 rounded-full bg-[#f5f5f5] flex items-center justify-center"><X size={16} className="text-[#737373]" /></button>
                    </div>
                    <div className="px-3 py-2">{config.models.map(m => (
                      <button key={m.value} onClick={() => { setModel(m.value); setSheet(null); }} className={`w-full flex items-center justify-between px-4 py-3.5 rounded-xl my-0.5 ${model === m.value ? 'bg-[#f5f5f5] font-semibold' : ''}`}>
          ...
          {config.durations && (
            <div>
              <label className="text-xs font-semibold text-[#999] mb-2 block">时长</label>
              <div className="mobile-scroll-x -mx-1"><div className="flex gap-2 px-1">
                {config.durations.map(d => <button key={d.value} onClick={() => setDuration(d.value)} className={`mobile-tap flex-shrink-0 px-5 py-2.5 rounded-xl text-xs font-medium transition-all ${duration === d.value ? 'bg-[#171717] text-white shadow-sm' : 'bg-white text-[#737373] border border-[#eee]'}`}>{d.label}</button>)}
              </div></div>
            </div>
          )}
          ...
          {/* Quantity */}
          <div>
            <label className="text-xs font-semibold text-[#999] mb-2 block">数量</label>
            <div className="flex items-center gap-3">
              <button onClick={() => setQuantity(p => Math.max(1, p - 1))} className="w-10 h-10 rounded-xl bg-white border border-[#eee] flex items-center justify-center text-lg font-medium">-</button>
              <span className="w-10 text-center text-base font-semibold">{quantity}</span>
              <button onClick={() => setQuantity(p => Math.min(3, p + 1))} className="w-10 h-10 rounded-xl bg-white border border-[#eee] flex items-center justify-center text-lg font-medium">+</button>
            </div>
          </div>

          {/* Prompt */}
          <div>
            <label className="text-xs font-semibold text-[#999] mb-2 block">描述</label>
            <textarea value={prompt} onChange={e => setPrompt(e.target.value)} placeholder="描述视频内容..." rows={3} className="w-full px-4 py-3 bg-white rounded-xl border border-[#eee] text-sm resize-none outline-none" />
          </div>

          {/* Generate */}
          {(isPolling || taskId) && (
            <div className="bg-amber-50 border border-amber-100 rounded-2xl px-4 py-4 flex items-center gap-3">
              <Loader2 size={18} className="text-amber-500 animate-spin" />
              <div><p className="text-sm font-medium text-amber-700">视频生成中</p><p className="text-xs text-amber-500">通常需要1-3分钟，请耐心等待...</p></div>
            </div>
          )}

          <button onClick={isAuthenticated ? handleGenerate : () => window.dispatchEvent(new Event('mobile-auth-required'))} disabled={isGenerating || isPolling || !!taskId} className="w-full flex items-center justify-center gap-2.5 py-4 rounded-2xl text-sm font-bold bg-[#171717] text-white active:bg-[#333] transition-all shadow-sm disabled:opacity-50">
          ...
          {results.length > 0 && <div>
            <div className="flex items-center gap-2 mb-3"><Film size={16} className="text-[#171717]" /><h2 className="text-sm font-bold text-[#171717]">生成视频</h2></div>
            <div className="space-y-3">{results.map((url, i) => (
              <div key={i} className="mobile-card overflow-hidden">
                <div className="aspect-[9/16] bg-black flex items-center justify-center">
                  <video src={url} controls className="w-full h-full object-contain" />
                </div>
                <div className="flex px-3 py-2.5 border-t border-[#f5f5f5]">
                  <button onClick={() => handleDownload(url)} className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg bg-[#f5f5f5] text-xs font-medium"><Download size={14} /> 下载</button>
                </div>
              </div>
            ))}</div>
          </div>}
