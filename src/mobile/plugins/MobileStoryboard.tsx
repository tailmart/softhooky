  ...
  return (
    <div className="flex flex-col h-full bg-white">
      <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-200 bg-white flex-shrink-0">
        ...
        <h1 className="text-base font-bold text-[#171717]">故事板</h1>
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="px-4 pt-4 pb-32 space-y-4">
          <p className="text-xs text-gray-400">输入剧本或创意描述，AI 将生成故事板分镜画面</p>

          <div>
            <label className="text-xs font-semibold text-gray-500 mb-2 block">剧本 / 创意描述 <span className="text-red-400">*</span></label>
            <textarea value={script} onChange={e => setScript(e.target.value)} placeholder="输入剧本内容或创意描述...&#10;例如：一个年轻人在城市中奔跑，穿越街道、公园，最终到达海边..." rows={6} className="w-full px-4 py-3 bg-gray-50 rounded-xl border border-gray-200 text-sm text-[#171717] placeholder-gray-300 resize-none outline-none leading-relaxed focus:border-blue-500/30 transition-colors" />
          </div>

          <div>
            <label className="text-xs font-semibold text-gray-500 mb-2 block">分镜数量</label>
            <div className="flex gap-2">{SHOT_COUNTS.map(n => <button key={n} onClick={() => setShotCount(n)} className={`mobile-tap flex-1 py-2.5 rounded-xl text-sm font-medium transition-all ${shotCount === n ? 'bg-blue-500 text-white shadow-sm shadow-blue-500/25' : 'bg-gray-50 text-gray-500 border border-gray-200'}`}>{n}镜</button>)}</div>
          </div>

          <button onClick={handleAnalyze} disabled={!script.trim() || isAnalyzing} className="w-full flex items-center justify-center gap-2.5 py-3.5 rounded-2xl text-sm font-semibold transition-all bg-gray-100 text-[#171717] border border-gray-200 active:bg-gray-200 disabled:opacity-30">
            {isAnalyzing ? <><Loader2 size={16} className="animate-spin" /> AI 分析中...</> : <><Sparkles size={16} /> AI 分析剧本</>}
          </button>

          {analysis && <div className="bg-gray-50 rounded-2xl border border-gray-200 p-4">
            <div className="flex items-center gap-2 mb-2"><Sparkles size={14} className="text-blue-400" /><span className="text-xs font-semibold text-gray-600">分镜方案</span></div>
            <p className="text-sm text-gray-500 leading-relaxed whitespace-pre-wrap">{analysis}</p>
          ...
          {results.length > 0 && <div>
            <div className="flex items-center gap-2 mb-3"><ImageIcon size={16} className="text-gray-500" /><h2 className="text-sm font-bold text-gray-600">分镜结果</h2></div>
            <div className="grid grid-cols-2 gap-3">{results.map((url, i) => (
              <div key={i} className="mobile-card overflow-hidden rounded-xl bg-gray-50 border border-gray-200">
                <div className="aspect-video bg-gray-50"><img src={url} className="w-full h-full object-contain cursor-pointer" onClick={() => setExpandedImage(url)} loading="lazy" /></div>
                <div className="flex px-3 py-2.5 border-t border-gray-100"><button onClick={() => handleDownload(url)} className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg bg-gray-100 text-gray-500 text-xs font-medium"><Download size={14} /> 下载</button></div>
              </div>
            ))}</div>
          </div>}
        </div>
