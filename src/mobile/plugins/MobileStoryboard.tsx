...
以JSON数组格式输出：[{ "shot": 1, "description": "...", "scale": "...", "angle": "..." }]`;
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      for (const url of urls) { try { await imageLibraryService.saveToLibrary({ image_url: url, prompt: '故事板生成', model: 'nanobann2', aspect_ratio: '16:9', type: 'generated' }); } catch {} }
    ...
    <div className="flex flex-col h-full bg-[#FAFAFA]">
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="px-4 pt-4 pb-32 space-y-4">
          <p className="text-xs text-[#a3a3a3]">输入剧本或创意描述，AI 将生成故事板分镜画面</p>

          <div>
            <label className="text-xs font-semibold text-[#999] mb-2 block">剧本 / 创意描述 <span className="text-red-400">*</span></label>
            <textarea value={script} onChange={e => setScript(e.target.value)} placeholder="输入剧本内容或创意描述...&#10;例如：一个年轻人在城市中奔跑，穿越街道、公园，最终到达海边..." rows={6} className="w-full px-4 py-3 bg-white rounded-xl border border-[#eee] text-sm resize-none outline-none leading-relaxed" />
          </div>

          <div>
            <label className="text-xs font-semibold text-[#999] mb-2 block">分镜数量</label>
            <div className="flex gap-2">{SHOT_COUNTS.map(n => <button key={n} onClick={() => setShotCount(n)} className={`mobile-tap flex-1 py-2.5 rounded-xl text-sm font-medium transition-all ${shotCount === n ? 'bg-[#171717] text-white shadow-sm' : 'bg-white text-[#737373] border border-[#eee]'}`}>{n}镜</button>)}</div>
          </div>

          <button onClick={handleAnalyze} disabled={!script.trim() || isAnalyzing} className="w-full flex items-center justify-center gap-2.5 py-3.5 rounded-2xl text-sm font-semibold transition-all bg-white text-[#171717] border border-[#ddd] active:bg-[#f5f5f5] disabled:opacity-40">
            {isAnalyzing ? <><Loader2 size={16} className="animate-spin" /> AI 分析中...</> : <><Sparkles size={16} /> AI 分析剧本</>}
          </button>

          {analysis && <div className="bg-white rounded-2xl border border-[#eee] p-4">
            <div className="flex items-center gap-2 mb-2"><Sparkles size={14} className="text-[#171717]" /><span className="text-xs font-semibold">分镜方案</span></div>
            <p className="text-sm text-[#525252] leading-relaxed whitespace-pre-wrap">{analysis}</p>
            <button onClick={handleGenerate} disabled={isGenerating || !isAuthenticated} className="mt-3 w-full flex items-center justify-center gap-2.5 py-3.5 rounded-2xl text-sm font-bold bg-[#171717] text-white active:bg-[#333] transition-all shadow-sm">
          ...
          {results.length > 0 && <div>
            <div className="flex items-center gap-2 mb-3"><ImageIcon size={16} className="text-[#171717]" /><h2 className="text-sm font-bold text-[#171717]">分镜结果</h2></div>
            <div className="grid grid-cols-2 gap-3">{results.map((url, i) => (
              <div key={i} className="mobile-card overflow-hidden">
                <div className="aspect-video bg-[#fafafa]"><img src={url} className="w-full h-full object-contain cursor-pointer" onClick={() => setExpandedImage(url)} loading="lazy" /></div>
