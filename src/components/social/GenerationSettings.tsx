...
import { Globe, Wand2, ChevronDown, Images } from 'lucide-react';
...
export const GenerationSettings: React.FC<GenerationSettingsProps> = ({
  ...
  return (
    <>
      ...
      {/* 生成张数 */}
      {showImageCount && onImageCountChange && (
        <div className="bg-white rounded-2xl p-4 border border-gray-200 shadow-sm">
          <div className="flex items-center gap-2 mb-3">
          ...
          <div className="grid grid-cols-4 gap-2">
            {[{ v: 0, l: 'AI自动' }, { v: 3, l: '3张' }, { v: 5, l: '5张' }].map(n => (
              <button
                key={n.v}
                onClick={() => onImageCountChange(n.v)}
                className={`py-2 rounded-xl text-xs font-medium transition-all ${imageCount === n.v ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}
              >
                {n.l}
              </button>
            ))}
            ...
            {aspectRatios.map(r => {
              ...
              return (
                ...
                >
                  {r.label}
                  {r.platform && (
                    <span className={`block text-[9px] ${selected ? 'text-white/70' : 'text-gray-400'}`}>
                      {r.platform}
                    </span>
                  )}
                </button>
        ...
        <div className="grid grid-cols-2 gap-2">
          {['2K', '4K'].map(q => (
            <button
              ...
              className={`py-2 rounded-xl text-xs font-medium transition-all ${quality === q ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}
            >
              {q}
            </button>
          ))}
